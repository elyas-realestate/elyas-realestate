...
import {
  Share2, QrCode, X, Facebook, Linkedin, Twitter, MessageCircle,
...
export default function ProfileCardClient({ card, links, identity, slug, testimonials }: any) {
  ...
  const topSocials = enabledLinks.filter(l => {
  ...
  return (
    <div dir="rtl" style={{
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
        <button onClick={() => setShareOpen(true)} aria-label="مشاركة" style={iconBtnStyle(card.text_color)}>
          <Share2 size={16} />
        </button>
        <button onClick={() => setQrOpen(true)} aria-label="رمز QR" style={iconBtnStyle(card.text_color)}>
          <QrCode size={16} />
        </button>
      </div>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          <div style={{
            margin: "0 auto 14px", display: "flex", justifyContent: "center",
            ...
            {avatarUrl ? (
              ...
              <InitialsAvatar name={displayName} bg={accent} color={card.bg_color || "#FAF7F2"} size={100} />
            )}
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 6, lineHeight: 1.3 }}>{displayName}</h1>
        ...
        {card.show_powered_by && (
          <div style={{
            ...
            <Link href="/" style={{
              ...
              background: accent, color: card.text_color || "#1A1206",
      ...
      {shareOpen && <ShareModal url={profileUrl} name={displayName} avatarUrl={avatarUrl} cardAccent={accent} cardBg={card.bg_color || "#FAF7F2"} onClose={() => setShareOpen(false)} bgColor={card.bg_color} textColor={card.text_color} />}
      {qrOpen && <QRModal url={profileUrl} name={displayName} avatarUrl={avatarUrl} cardAccent={accent} cardBg={card.bg_color || "#FAF7F2"} onClose={() => setQrOpen(false)} bgColor={card.bg_color} textColor={card.text_color} />}
      {contactFormOpen && <ContactFormModal link={contactFormOpen} slug={slug} onClose={() => setContactFormOpen(null)} bgColor={card.bg_color} textColor={card.text_color} accent={accent} />}
    </div>
  );
}
...
// ─────────────────────────────────────────────────────────────
function ElementCard({ link, cardTextColor, cardBgColor, accent, onContactFormOpen }: {
  link: ProfileLink; cardTextColor: string; cardBgColor: string; accent: string;
  ...
  return (
    <Modal onClose={onClose} bgColor={bgColor} textColor={textColor} title={`شارك رابط ${name}`}>
      <div style={{ textAlign: "center", padding: "0 8px" }}>
        ...
        <div style={{
          background: `${textColor || "#1A1206"}08`, border: `1px solid ${textColor || "#1A1206"}15`,
          borderRadius: 100, padding: "10px 16px", fontSize: 12, marginBottom: 18, direction: "ltr",
        }}>{url}</div>
...
function QRModal({ url, name, avatarUrl, cardAccent, cardBg, onClose, bgColor, textColor }: any) {
  ...
  function downloadQR() {
    ...
    a.href = qrDataUrl; a.download = `${name}-qr.png`;
    ...
    <Modal onClose={onClose} bgColor={bgColor} textColor={textColor} title="شارك رمز QR للبروفايل">
      <div style={{ textAlign: "center", padding: "0 8px" }}>
        ...
        <div style={{ marginTop: 8, display: "flex", justifyContent: "center", gap: 16, fontSize: 11, opacity: 0.65 }}>
          <span>شارك</span><span>حفظ QR</span>
        </div>
      </div>
    </Modal>
  );
...
function ContactFormModal({ link, slug, onClose, bgColor, textColor, accent }: any) {
    ...
    return (
      <Modal onClose={onClose} bgColor={bgColor} textColor={textColor} title="تم الإرسال">
        <div style={{ textAlign: "center", padding: "20px 8px" }}>
          ...
          <div style={{ fontSize: 13, opacity: 0.7, marginTop: 6 }}>سيتواصل معك صاحب البطاقة قريباً</div>
        </div>
      </Modal>
    );
  }
  ...
  return (
    <Modal onClose={onClose} bgColor={bgColor} textColor={textColor} title={meta.title || link.label}>
      <div style={{ padding: "0 8px" }}>
        {meta.description && (
          <p style={{ fontSize: 13, opacity: 0.75, marginBottom: 14 }}>{meta.description}</p>
        )}
        <FormField label="الاسم *" value={name} onChange={setName} />
        {askPhone && <FormField label="الجوال" value={phone} onChange={setPhone} type="tel" dir="ltr" />}
        {askEmail && <FormField label="البريد الإلكتروني" value={email} onChange={setEmail} type="email" dir="ltr" />}
        {askMessage && <FormField label="الرسالة" value={message} onChange={setMessage} multiline />}
        <button onClick={submit} disabled={!name.trim()}
          style={{
            width: "100%", padding: "12px 16px", borderRadius: 100,
            background: accent, color: "#FFFFFF", fontWeight: 700, fontSize: 14,
            border: "none", cursor: name.trim() ? "pointer" : "not-allowed", opacity: name.trim() ? 1 : 0.5,
            marginTop: 12,
          }}>
          إرسال
        </button>
...
function FormField({ label, value, onChange, type = "text", multiline, dir }: any) {
  return (
    <div style={{ marginBottom: 12 }}>
      <label style={{ fontSize: 12, opacity: 0.75, display: "block", marginBottom: 4 }}>{label}</label>
      {multiline ? (
        <textarea value={value} onChange={e => onChange(e.target.value)} rows={3}
          ...
          style={{
            ...
            fontSize: 13, fontFamily: "inherit", color: "inherit", resize: "vertical",
          }} />
      ) : (
