          ...
          <div className="fade-up" style={{ position:"relative", zIndex:1, textAlign:"center", maxWidth:820, padding:"0 28px" }}>
            {brokerPhoto && (
              <div className="d1 fade-up" style={{ display:"inline-block", marginBottom:24 }}>
                <img src={brokerPhoto} alt={name} style={{ width:96, height:96, borderRadius:"50%", objectFit:"cover", border:`3px solid color-mix(in srgb, ${clrAccent} 35%, transparent)`, boxShadow:`0 0 0 6px color-mix(in srgb, ${clrAccent} 8%, transparent)` }} />
              </div>
            )}
            ...
            <h1 className="font-kufi d2 fade-up" style={{ fontSize:fntHero, fontWeight:900, lineHeight:1.25, marginBottom:20, color:clrTextPrimary }}>{name}</h1>
            ...
            <div className="d4 fade-up hero-actions" style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12 }}>
              ...
              <a href="#request-form" className="accent-bg" style={{ color:clrBgPrimary, textDecoration:"none", fontSize:14, fontWeight:700, padding:"14px 24px", borderRadius:11, display:"flex", alignItems:"center", gap:8, opacity:0.9 }}>طلب عقار</a>
            </div>
          ...
          <section id="properties" style={{ padding:"90px 48px", background:clrBgSecondary }} className="section-pad">
            <div style={{ maxWidth:1100, margin:"0 auto" }}>
              <div style={{ textAlign:"center", marginBottom:48 }}>
                <div className="accent" style={{ fontSize:12, fontWeight:700, letterSpacing:2, marginBottom:14 }}>— عقارات ومشاريع مختارة —</div>
              ...
              {(["مشروع", "بيع", "إيجار"] as const).map(type => {
                const filtered = type === "مشروع"
                  ? properties.filter((p: any) => p.offer_type === "مشروع" || (p.sub_category || "").includes("مشروع"))
                  : type === "إيجار"
          ...
          <section id="request-form" style={{ padding:"90px 48px", background:clrBgPrimary }} className="section-pad">
            <div style={{ maxWidth:680, margin:"0 auto" }}>
              <div style={{ textAlign:"center", marginBottom:48 }}>
                ...
                <h2 className="font-kufi" style={{ fontSize:fntSection, fontWeight:900, color:clrTextPrimary, lineHeight:1.3, marginBottom:12 }}>أخبرني عن العقار الذي تبحث عنه</h2>
                <p style={{ fontSize:fntBody, color:clrTextSec, lineHeight:1.8 }}>سأبحث لك عن أفضل الخيارات وأتواصل معك مباشرة.</p>
              </div>
              <RequestForm tenantId={tenantId} accentColor={clrAccent} accentDark={clrAccentDark} bgCard={clrBgCard} bgPrimary={clrBgPrimary} textPrimary={clrTextPrimary} textSecondary={clrTextSec} fontBody={fntBody} />
            </div>
          </section>
        )}
        ...
        <footer style={{ padding:"32px 48px", background:clrBgPrimary, borderTop:`1px solid color-mix(in srgb, ${clrAccent} 8%, transparent)` }}>
          <div style={{ maxWidth:1100, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              {identity?.fal_license && <span style={{ fontSize:11, color:clrTextMuted, borderRight:`1px solid color-mix(in srgb, ${clrAccent} 12%, transparent)`, paddingRight:8 }}>رخصة فال: {identity.fal_license}</span>}
            </div>
