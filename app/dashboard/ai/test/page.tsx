...
export default function TestTab() {
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <h3 className="mb-3 text-sm font-bold" style={{ color: "var(--gold-2)" }}>
          ١) اختر المساعد ({employees.filter((e) => e.is_active).length}/{employees.length} نشط)
        </h3>
        {loadingList ? (
          <div className="flex justify-center py-6">
            <Loader2 size={18} className="animate-spin" style={{ color: "var(--gold-2)" }} />
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="mb-1 flex items-center gap-2">
                    <Bot
                      size={13}
                      style={{ color: isActive ? "var(--gold-2)" : "var(--text-soft)" }}
                    />
        ...
        >
          ...
          {result.error && (
            <div
              ...
              style={{ background: "var(--bg-surface-1)", color: "var(--danger)" }}
            >
              {result.error}
            </div>
          )}
          {result.output && (
            <div>
              <div className="mb-2 text-xs font-bold" style={{ color: "var(--text-soft)" }}>
                📤 رد المساعد:
              </div>
              <div
