...
const STAGES = [
  ...
  { id: "تفاوض", color: "#EAB308", bg: "rgba(234,179,8,0.12)", light: "rgba(234,179,8,0.06)" },
  {
    ...
    light: "rgba(167,139,250,0.06)",
  },
  {
    id: "مكتملة",
    color: "var(--success)",
    bg: "rgba(74,222,128,0.12)",
    light: "rgba(74,222,128,0.06)",
  },
  {
    id: "ملغاة",
    color: "var(--danger)",
    bg: "rgba(248,113,113,0.12)",
    light: "rgba(248,113,113,0.06)",
  },
];

const PRIORITY_COLOR: Record<string, string> = {
  منخفض: "var(--text-disabled)",
  متوسط: "var(--gold-2)",
  مرتفع: "#EAB308",
  عاجل: "var(--danger)",
};
...
export default function Deals() {
  ...
  async function onDrop(stageId: string) {
    ...
    if (!deal || deal.current_stage === stageId) {
      setDragging(null);
      setDragOver(null);
      return;
    }
  ...
  const filtered = deals.filter(
    (d) => !search || d.title?.includes(search) || d.deal_type?.includes(search)
  );

  const totalValue = deals
    .filter((d) => d.current_stage === "مكتملة")
    .reduce((s, d) => s + (d.target_value || 0), 0);
  const pipelineValue = deals
    .filter((d) => !["مكتملة", "ملغاة"].includes(d.current_stage))
    .reduce((s, d) => s + (d.target_value || 0), 0);

  const inp =
    "w-full bg-[var(--bg-surface-2)] border border-[var(--gold-bg-hover)] rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[var(--gold-2)] text-[var(--text-strong)]";
  ...
  return (
    <div dir="rtl">
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="mb-1 text-2xl font-bold">الصفقات</h2>
          <p className="text-sm" style={{ color: "var(--text-faint)" }}>
            {deals.length} صفقة · pipeline{" "}
            <span style={{ color: "var(--gold-2)" }}>
              {formatSAR(pipelineValue, { short: true })}
            </span>{" "}
            · مكتملة{" "}
            <span style={{ color: "var(--success)" }}>
              {formatSAR(totalValue, { short: true })}
            </span>
          </p>
        </div>
          ...
          >
            <button
              ...
              style={{
                padding: "6px 10px",
                borderRadius: 7,
                background: view === "kanban" ? "var(--gold-bg-hover)" : "transparent",
                color: view === "kanban" ? "var(--gold-2)" : "var(--text-faint)",
                border: "none",
                cursor: "pointer",
                display: "flex",
                alignItems: "center",
            ...
            <button
              ...
              style={{
                padding: "6px 10px",
                borderRadius: 7,
                background: view === "list" ? "var(--gold-bg-hover)" : "transparent",
                color: view === "list" ? "var(--gold-2)" : "var(--text-faint)",
                border: "none",
                cursor: "pointer",
                display: "flex",
                alignItems: "center",
      ...
      {showAdd && (
        <form
          ...
          className="mb-6 rounded-xl border border-[var(--gold-bg)] bg-[var(--bg-surface-1)] p-5"
        >
          <h3
            className="mb-4 font-bold"
            style={{ color: "var(--gold-2)", fontSize: 12, letterSpacing: 1 }}
          >
            صفقة جديدة
          </h3>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <label className="mb-2 block text-sm text-[var(--text-soft)]">عنوان الصفقة *</label>
              <input
                name="title"
                value={form.title}
            ...
            <div>
              <label className="mb-2 block text-sm text-[var(--text-soft)]">نوع الصفقة</label>
              <select
                className={inp}
            ...
            <div>
              <label className="mb-2 block text-sm text-[var(--text-soft)]">المرحلة</label>
              <select
                className={inp}
            ...
            <div>
              <label className="mb-2 block text-sm text-[var(--text-soft)]">العقار</label>
              <select
                className={inp}
            ...
            <div>
              <label
                className="mb-2 block text-sm text-[var(--text-soft)]"
                        ...
                        onClick={() =>
                          setForm((f) => ({
                            ...
                            expected_commission: String(
                              Math.round((Number(f.target_value) * pct) / 100)
                            ),
                          }))
                        }
                        style={{
              ...
              <input
                ...
                placeholder="أو اختر نسبة أعلاه"
              />
            </div>
            <div>
              <label className="mb-2 block text-sm text-[var(--text-soft)]">
            ...
            <div>
              <label className="mb-2 block text-sm text-[var(--text-soft)]">الإجراء التالي</label>
              <input
                className={inp}
            ...
            <div className="sm:col-span-2">
              <label className="mb-2 block text-sm text-[var(--text-soft)]">ملخص</label>
              <textarea
                rows={2}
                className={inp}
                value={form.summary}
            ...
            <div className="flex gap-3 sm:col-span-2">
              <button
                type="submit"
                className="rounded-lg bg-[var(--gold-2)] px-6 py-2.5 text-sm font-bold text-[var(--bg-page)]"
              >
                حفظ
              </button>
              <button
      ...
      <div className="relative mb-5">
        <Search size={16} className="absolute top-3.5 right-3 text-[var(--text-soft)]" />
        <input
          type="text"
          placeholder="ابحث عن صفقة..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full rounded-lg border border-[var(--gold-bg)] bg-[var(--bg-surface-1)] py-3 pr-10 pl-4 text-sm text-[var(--text-strong)] focus:border-[var(--gold-2)] focus:outline-none"
        />
      </div>
          ...
          {STAGES.map((stage) => {
            ...
            return (
              ...
              >
                {/* Column header */}
                <div
                  style={{ padding: "14px 16px 10px", borderBottom: `1px solid ${stage.color}18` }}
                >
                  <div
                    style={{
                      display: "flex",
                  ...
                  {stageDeals.length === 0 && (
                    <div
                      style={{
                        ...
                        height: 60,
                        fontSize: 12,
                        color: isDragTarget ? stage.color : "var(--border-1)",
                      }}
                    >
      ...
      filtered.length === 0 ? (
        <div className="py-20 text-center">
          ...
          <button
            ...
            className="rounded-xl px-6 py-3 text-sm font-bold text-[var(--bg-page)]"
              ...
              >
                <div
                  style={{
                    ...
                    height: 8,
                    borderRadius: "50%",
                    background: stage?.color || "var(--text-disabled)",
                    flexShrink: 0,
                  }}
                />
                <div style={{ flex: 1, minWidth: 120 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-on-dark)" }}>
                    {deal.title}
                  </div>
                  {deal.next_action && (
                ...
                <span
                  style={{
                    fontSize: 11,
                    color: stage?.color || "var(--text-disabled)",
                    background: stage?.bg || "transparent",
                    padding: "3px 10px",
                    borderRadius: 99,
                    whiteSpace: "nowrap",
                  }}
                ...
                {deal.priority && (
                  <span
                    style={{
                      fontSize: 11,
                      color: PRIORITY_COLOR[deal.priority] || "var(--text-disabled)",
                      whiteSpace: "nowrap",
                    }}
                  >
                    {deal.priority}
...
function KanbanCard({
  ...
  return (
    ...
    >
      ...
      {daysLeft !== null && (
        <div
          style={{
            fontSize: 10,
            color:
              daysLeft < 0 ? "var(--danger)" : daysLeft <= 3 ? "#EAB308" : "var(--text-disabled)",
            marginBottom: 8,
            display: "flex",
            alignItems: "center",
            gap: 4,
      ...
      {/* Next action */}
      {deal.next_action && (
        <div style={{ fontSize: 10, color: "var(--text-faint)", marginBottom: 8, lineHeight: 1.4 }}>
          {deal.next_action}
        </div>
      )}
