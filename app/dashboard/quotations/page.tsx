...
export default function QuotationsPage() {
  ...
  if (missingTable) return (
    <div dir="rtl" style={{ maxWidth: 520, margin: "60px auto", textAlign: "center" }}>
      ...
      <h2 style={{ fontSize: 18, fontWeight: 700, color: "var(--text-strong)", marginBottom: 12 }}>يلزم تفعيل جدول عروض الأسعار</h2>
    ...
    <div dir="rtl" className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p style={{ color: "var(--text-faint)", fontSize: 13 }}>إنشاء وإدارة عروض الأسعار للعملاء</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "قيمة المقبولة", val: fmtNum(kpis.totalAmount) + " ر.س", icon: SARIcon, color: "var(--gold-2)", isSar: true },
        ].map((k, i) => (
      ...
      {showForm && (
        <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface-1)", border: "1px solid rgba(198,145,76,0.18)" }}>
          <div className="flex items-center justify-between mb-5">
            <h3 style={{ fontSize: 13, fontWeight: 700, color: "var(--gold-2)", letterSpacing: 1 }}>عرض سعر جديد</h3>
      ...
      {quotations.length === 0 ? (
        <div className="rounded-2xl py-20 text-center" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg-soft)" }}>
          <FileText size={40} style={{ color: "var(--gold-bg-hover)", margin: "0 auto 14px", display: "block" }} />
          <p style={{ color: "var(--text-faint)", fontSize: 14, marginBottom: 16 }}>لا توجد عروض أسعار — أنشئ أول عرض</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <h4 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-on-dark)" }}>{q.title}</h4>
                      ...
                      {isExpired && (
                        <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 6, background: "rgba(248,113,113,0.08)", color: "var(--danger)" }}>منتهي الصلاحية</span>
                      )}
                    </div>
                  ...
                  <div className="text-left">
                    <p className="font-cairo font-bold" style={{ fontSize: 18, color: "var(--gold-2)" }}>
                ...
                <div className="flex gap-2 mt-4 pt-3 flex-wrap" style={{ borderTop: "1px solid var(--gold-bg-soft)" }}>
                  {q.status === "مسودة" && (
                    <button onClick={() => updateStatus(q.id, "مُرسل")}
                      ...
                      style={{ background: "rgba(96,165,250,0.08)", color: "var(--info)", border: "1px solid rgba(96,165,250,0.2)", cursor: "pointer" }}>
                      <Send size={11} /> إرسال
                    </button>
                  )}
                  {q.status === "مُرسل" && (
                    <>
                      ...
                      <button onClick={() => updateStatus(q.id, "مرفوض")}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition"
                        style={{ background: "rgba(248,113,113,0.08)", color: "var(--danger)", border: "1px solid rgba(248,113,113,0.2)", cursor: "pointer" }}>
                        <XCircle size={11} /> رفض
                      </button>
                    </>
                  )}
