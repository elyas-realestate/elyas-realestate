...
export default function WhatsAppSettings() {
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 80 }}>
        <Loader2
          size={28}
          style={{ color: "var(--success-2)", animation: "spin 1s linear infinite" }}
        />
  ...
  return (
    <div>
      ...
      >
        <SettingsIcon size={20} style={{ color: "var(--success-2)" }} /> إعدادات WhatsApp Business
        API
      </h1>
      ...
      >
        {configured ? (
          ...
          <AlertCircle size={18} style={{ color: "var(--gold-1)" }} />
        )}
        <div style={{ flex: 1 }}>
          <div
            style={{
              fontSize: 13,
              fontWeight: 700,
              color: configured ? "var(--success)" : "var(--gold-1)",
            }}
          >
            {configured && config.is_active
              ? "WhatsApp Business مفعَّل ويرسل عبر Meta"
              : configured
                ? "الإعدادات محفوظة — فعّل الخدمة لبدء الإرسال"
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          >
            ...
            <Field label="Business Account ID" hint="WhatsApp Business Account ID">
              <input
                ...
                onChange={(e) => setConfig((c) => ({ ...c, business_account_id: e.target.value }))}
                style={inputStyle}
            ...
            <Field label="Access Token (دائم)" hint="System User Access Token من Meta">
              <div style={{ position: "relative" }}>
                <input
                  ...
                  placeholder={
                    config.access_token_enc ? "•••••••• (محفوظ، اتركه فارغاً للإبقاء)" : "EAAGm..."
                  }
                />
                <button
                  type="button"
            ...
            <Field label="رقم WhatsApp المعروض للعملاء" hint="بصيغة دولية">
              <input
                ...
                onChange={(e) => setConfig((c) => ({ ...c, display_phone: e.target.value }))}
                style={inputStyle}
        ...
        <Card title="Webhook (لاستقبال الرسائل والردود التلقائية)" icon={Webhook}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12 }}>
            <Field
              label="رابط الـ Webhook (انسخه إلى Meta)"
              hint="ضعه في Meta Business Manager → Webhooks → Callback URL"
            >
              <div style={{ display: "flex", gap: 8 }}>
                ...
                />
                <button onClick={() => copy(webhookUrl, "الرابط")} style={btnInline}>
                  نسخ
                </button>
              </div>
            </Field>
            <Field
              label="Verify Token"
              hint="نص عشوائي تختاره أنت — يجب أن يطابق نفس القيمة في Meta + متغير البيئة META_WEBHOOK_VERIFY_TOKEN"
            >
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  value={config.webhook_verify_token || ""}
                  onChange={(e) =>
                    setConfig((c) => ({ ...c, webhook_verify_token: e.target.value }))
                  }
                  style={inputStyle}
            ...
            >
              <div
                style={{ fontSize: 11, color: "var(--gold-1)", fontWeight: 600, marginBottom: 4 }}
              >
                ⚠ تذكير
              </div>
        ...
        <Card title="الرد التلقائي بالذكاء الاصطناعي" icon={Bot}>
          ...
          >
            ...
            >
              تفعيل الرد التلقائي — يُرد على رسائل العملاء الواردة باستخدام AI، ويبحث في عقاراتك
              المنشورة لاقتراح ٣ مطابقات.
            </label>
          </div>
          <div
            style={{
        ...
        <Card title="حالة التفعيل" icon={CheckCircle2}>
          ...
          >
            ...
            <span style={{ flex: 1, fontSize: 13, color: "var(--text-on-dark)", lineHeight: 1.7 }}>
              تفعيل WhatsApp Business — كل الرسائل تُرسل عبر Meta API. عند التعطيل، يستخدم النظام
              wa.me كبديل.
            </span>
          </label>
        </Card>
...
function Field({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      ...
      {children}
      {hint && (
        <span style={{ fontSize: 10, color: "var(--text-disabled)", lineHeight: 1.6 }}>{hint}</span>
      )}
    </div>
  );
}
