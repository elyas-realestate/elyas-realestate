  ...
  return (
    <div
      style={{
        position: "fixed",
        inset: 0,
        background: "var(--modal-overlay-2)",
        zIndex: 50,
        display: "flex",
        alignItems: "center",
...
export default function TenantPortalPage() {
  ...
  async function markPaid(id: string) {
    ...
    setPayments((prev) =>
      prev.map((p) =>
        p.id === id
          ? { ...p, status: "paid", paid_date: new Date().toISOString().split("T")[0] }
          : p
      )
    );
  }
  ...
  async function updateMaintStatus(id: string, status: string) {
    ...
    setMaintenance((prev) =>
      prev.map((m) => (m.id === id ? { ...m, status: status as MaintenanceRequest["status"] } : m))
    );
  }
    ...
    <div style={{ padding: "24px 28px", fontFamily: "inherit", direction: "rtl" }}>
      ...
      >
        <div>
          ...
          <p style={{ color: "var(--text-ghost)", fontSize: 14, marginTop: 4 }}>
            إدارة العقود والدفعات وطلبات الصيانة
          </p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          {tab === "contracts" && (
            <button onClick={() => setContractModal(true)} style={btnStyle()}>
              <Plus size={15} /> عقد جديد
            </button>
          )}
          {tab === "payments" && (
            <button onClick={() => setPaymentModal(true)} style={btnStyle()}>
              <Plus size={15} /> دفعة جديدة
            </button>
          )}
          {tab === "maintenance" && (
            <button onClick={() => setMaintModal(true)} style={btnStyle()}>
              <Plus size={15} /> طلب صيانة
            </button>
          )}
        </div>
      </div>
      ...
      >
        {[
          ...
          >
            <div
              style={{ color: s.color, background: s.color + "18", borderRadius: 10, padding: 10 }}
            >
              {s.icon}
            </div>
            <div>
      ...
      {loading ? (
        <div style={{ color: "var(--text-ghost)", padding: 40, textAlign: "center" }}>
          جاري التحميل…
        </div>
      ) : (
        <>
                  ...
                  >
                    <div>
                      ...
                      >
                        {client && (
                          <span>
                            العميل: <b style={{ color: "#CBD5E1" }}>{client.full_name}</b>
                          </span>
                        )}
                        {prop && (
                          <span>
                            العقار: <b style={{ color: "#CBD5E1" }}>{prop.title}</b>
                          </span>
                        )}
                        {c.monthly_rent && (
                          <span>
                            الإيجار الشهري:{" "}
                            <b style={{ color: "var(--gold-2)" }}>{fmt(c.monthly_rent)}</b>
                          </span>
                        )}
                      ...
                      {c.notes && (
                        <p style={{ color: "var(--text-ghost)", fontSize: 13, marginTop: 6 }}>
                          {c.notes}
                        </p>
                      )}
                    </div>
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      >
                        <span style={{ color: "var(--gold-2)", fontWeight: 700, fontSize: 16 }}>
                          {fmt(p.amount)}
                        </span>
                        <Badge
                          cfg={PAYMENT_STATUS[isLate && p.status === "pending" ? "late" : p.status]}
                        />
                      </div>
                      <div
                        style={{
                      ...
                      >
                        ...
                        {contract && (
                          <span>
                            العقد: <b style={{ color: "#CBD5E1" }}>{contract.title}</b>
                          </span>
                        )}
                        <span>
                        ...
                        {p.paid_date && (
                          <span>
                            ...
                            تاريخ الدفع: {fmtDate(p.paid_date)}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    >
                      <div>
                        ...
                        >
                          ...
                          {m.cost && (
                            <span>
                              التكلفة: <b style={{ color: "var(--gold-2)" }}>{fmt(m.cost)}</b>
                            </span>
                          )}
                      ...
                      {m.status !== "resolved" && m.status !== "cancelled" && (
                        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                          {m.status === "open" && (
                            <button
                              onClick={() => updateMaintStatus(m.id, "in_progress")}
                          ...
                          {m.status === "in_progress" && (
                            <button
                              onClick={() => updateMaintStatus(m.id, "resolved")}
                          ...
                          <button
                            onClick={() => updateMaintStatus(m.id, "cancelled")}
      ...
      {contractModal && (
        <ModalOverlay onClose={() => setContractModal(false)}>
          ...
          >
            ...
            <button onClick={() => setContractModal(false)} style={closeBtn()}>
              <X size={18} />
            </button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              ...
              <input
                ...
                style={inp()}
                placeholder="مثال: عقد إيجار شقة الملقا"
              />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div>
                {label("العميل")}
                <select
                  value={cForm.client_id}
                  onChange={(e) => setCForm((f) => ({ ...f, client_id: e.target.value }))}
                  style={inp()}
                >
            ...
            <div>
              {label("العقار")}
              <select
                value={cForm.property_id}
                onChange={(e) => setCForm((f) => ({ ...f, property_id: e.target.value }))}
                style={inp()}
              >
                <option value="">— اختر —</option>
                {properties.map((p) => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCForm((f) => ({ ...f, start_date: e.target.value }))}
                  style={inp()}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCForm((f) => ({ ...f, end_date: e.target.value }))}
                  style={inp()}
                />
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                ...
                <input
                  type="number"
                  value={cForm.monthly_rent}
                  onChange={(e) => setCForm((f) => ({ ...f, monthly_rent: e.target.value }))}
                  style={inp()}
                />
              </div>
              <div>
                {label("القيمة الإجمالية (ر.س)")}
                <input
                  type="number"
                  value={cForm.total_value}
                  onChange={(e) => setCForm((f) => ({ ...f, total_value: e.target.value }))}
                  style={inp()}
                />
              </div>
            </div>
            ...
            <button
              ...
              disabled={saving || !cForm.title}
              style={btnStyle("100%", saving || !cForm.title)}
            >
      ...
      {paymentModal && (
        <ModalOverlay onClose={() => setPaymentModal(false)}>
          ...
          >
            ...
            <button onClick={() => setPaymentModal(false)} style={closeBtn()}>
              <X size={18} />
            </button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                {label("العميل")}
                <select
                  value={pForm.client_id}
                  onChange={(e) => setPForm((f) => ({ ...f, client_id: e.target.value }))}
                  style={inp()}
                >
                  <option value="">— اختر —</option>
                  {clients.map((c) => (
              ...
              <div>
                {label("العقد")}
                <select
                  value={pForm.contract_id}
                  onChange={(e) => setPForm((f) => ({ ...f, contract_id: e.target.value }))}
                  style={inp()}
                >
                  <option value="">— اختر —</option>
                  {contracts.map((c) => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div>
                ...
                <input
                  type="date"
                  value={pForm.due_date}
                  onChange={(e) => setPForm((f) => ({ ...f, due_date: e.target.value }))}
                  style={inp()}
                />
              </div>
            </div>
            ...
            <button
              onClick={savePayment}
              disabled={saving || !pForm.amount || !pForm.due_date}
              style={btnStyle("100%", saving || !pForm.amount || !pForm.due_date)}
            >
      ...
      {maintModal && (
        <ModalOverlay onClose={() => setMaintModal(false)}>
          ...
          >
            ...
            <button onClick={() => setMaintModal(false)} style={closeBtn()}>
              <X size={18} />
            </button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setMForm((f) => ({ ...f, client_id: e.target.value }))}
                  style={inp()}
                >
                  <option value="">— اختر —</option>
                  {clients.map((c) => (
            ...
            <div>
              {label("العقار")}
              <select
                value={mForm.property_id}
                onChange={(e) => setMForm((f) => ({ ...f, property_id: e.target.value }))}
                style={inp()}
              >
                <option value="">— اختر —</option>
                {properties.map((p) => (
            ...
            <button
              onClick={saveMaint}
              disabled={saving || !mForm.title}
              style={btnStyle("100%", saving || !mForm.title)}
            >
