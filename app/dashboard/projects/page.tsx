...
function UnitRow({ unit, onUpdated, onDeleted }: { unit: any; onUpdated: () => void; onDeleted: () => void }) {
  ...
  if (!editing) return (
    <div className="flex items-center gap-3 rounded-xl px-4 py-3 transition"
      ...
      <div style={{ flex: 1, minWidth: 0, display: "grid", gridTemplateColumns: "80px 60px 70px 80px 90px 1fr", gap: 8, alignItems: "center" }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-on-dark)" }}>وحدة {unit.unit_number}</span>
        <span style={{ fontSize: 12, color: "var(--text-faint)" }}>{unit.unit_type || "—"}</span>
        <span style={{ fontSize: 12, color: "var(--text-faint)" }}>{unit.floor ? `دور ${unit.floor}` : "—"}</span>
        <span style={{ fontSize: 12, color: "var(--text-soft)" }}>{unit.area ? unit.area + " م²" : "—"}</span>
        <span style={{ fontSize: 12, fontWeight: 600, color: "var(--gold-2)" }}>{unit.price ? fmtNum(unit.price) + " ر.س" : "—"}</span>
      ...
      <div className="flex gap-1.5" style={{ flexShrink: 0 }}>
        <button onClick={() => setEditing(true)}
          style={{ width: 28, height: 28, borderRadius: 8, background: "var(--gold-bg-soft)", border: "1px solid var(--gold-bg)", color: "var(--gold-2)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <Edit3 size={11} />
        </button>
        <button onClick={del}
          style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.12)", color: "var(--danger)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <Trash2 size={11} />
        </button>
      </div>
    </div>
  ...
  return (
    <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(198,145,76,0.04)", border: "1px solid var(--gold-bg-hover)" }}>
            ...
            {Object.keys(UNIT_STATUS_CFG).map(s => (
              <button key={s} type="button" onClick={() => setForm(x => ({ ...x, status: s }))}
                ...
                style={{
                  ...
                  border: "1px solid " + (form.status === s ? UNIT_STATUS_CFG[s].color : "var(--gold-bg-soft)"),
                  cursor: "pointer",
                }}>
                {s}
              </button>
      ...
      <div className="flex gap-2">
        <button onClick={save} disabled={saving}
          ...
          style={{ background: "linear-gradient(135deg,var(--gold-2),var(--gold-3))", color: "var(--bg-page)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
          <Check size={14} /> {saving ? "..." : "حفظ"}
        </button>
        <button onClick={() => setEditing(false)}
          style={{ padding: "8px 16px", borderRadius: 12, background: "var(--bg-surface-2)", color: "var(--text-soft)", fontSize: 13, border: "1px solid var(--gold-bg)", cursor: "pointer" }}>
          إلغاء
        </button>
      </div>
    </div>
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg)" }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        {!project.main_image && (
          <div style={{ height: 80, background: "linear-gradient(135deg,#1A1208,var(--bg-surface-1))" }} />
        )}
      </div>
      ...
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h3 style={{ fontSize: 16, fontWeight: 700, color: "var(--text-on-dark)" }}>{project.name}</h3>
            ...
            {(project.city || project.district) && (
              <p style={{ fontSize: 12, color: "var(--text-faint)", marginTop: 4, display: "flex", alignItems: "center", gap: 3 }}>
          ...
          <button onClick={deleteProject}
            style={{ width: 30, height: 30, borderRadius: 8, background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.12)", color: "var(--danger)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
            <Trash2 size={13} />
          </button>
        </div>

        {project.description && (
          <p style={{ fontSize: 12, color: "var(--text-faint)", lineHeight: 1.7, marginBottom: 14 }}>{project.description}</p>
        )}

        {/* Meta */}
        <div className="flex gap-4 flex-wrap mb-4" style={{ fontSize: 11, color: "var(--text-faint)" }}>
          ...
          {project.location_url && (
            <a href={project.location_url} target="_blank" rel="noopener noreferrer"
              style={{ color: "var(--gold-2)", textDecoration: "none", display: "flex", alignItems: "center", gap: 3 }}>
        ...
        {stats.total > 0 && (
          <div className="flex gap-2 flex-wrap mb-4">
            ...
            {stats.revenue > 0 && (
              <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 20, background: "var(--gold-bg)", color: "var(--gold-2)", fontWeight: 700, display: "flex", alignItems: "center", gap: 3 }}>
        ...
        {stats.total > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div className="flex justify-between mb-1.5" style={{ fontSize: 10, color: "var(--text-faint)" }}>
              <span>نسبة المبيعات</span>
              <span style={{ color: "var(--gold-2)" }}>{soldPct}%</span>
            </div>
            <div style={{ height: 6, borderRadius: 999, background: "var(--bg-surface-3)", overflow: "hidden", display: "flex" }}>
              <div style={{ width: soldPct + "%", background: "var(--danger)", transition: "width 0.5s" }} />
              <div style={{ width: reservedPct + "%", background: "var(--warning)", transition: "width 0.5s" }} />
            </div>
      ...
      <button
        ...
        style={{
          ...
          color: "var(--gold-2)", fontSize: 12, fontWeight: 600,
        ...
        <div style={{ padding: "16px 20px 20px" }}>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowAddUnit(v => !v)}
              ...
              style={{ background: "linear-gradient(135deg,var(--gold-2),var(--gold-3))", color: "var(--bg-page)", fontSize: 12, fontWeight: 700, cursor: "pointer", border: "none" }}>
            ...
            <div className="rounded-xl p-4 mb-4 space-y-3" style={{ background: "rgba(198,145,76,0.04)", border: "1px solid rgba(198,145,76,0.18)" }}>
                  ...
                  <div className="flex gap-2">
                    {Object.keys(UNIT_STATUS_CFG).map(s => (
                      <button key={s} type="button" onClick={() => setNewUnit(x => ({ ...x, status: s }))}
                        ...
                        style={{ background: newUnit.status === s ? UNIT_STATUS_CFG[s].bg : "var(--bg-surface-2)", color: newUnit.status === s ? UNIT_STATUS_CFG[s].color : "var(--text-faint)", border: "1px solid " + (newUnit.status === s ? UNIT_STATUS_CFG[s].color : "var(--gold-bg-soft)"), cursor: "pointer" }}>
                        {s}
                      </button>
                    ))}
                  </div>
              ...
              <div className="flex gap-2">
                <button onClick={addUnit} disabled={addSaving}
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 20px", borderRadius: 12, background: "linear-gradient(135deg,var(--gold-2),var(--gold-3))", color: "var(--bg-page)", fontSize: 13, fontWeight: 700, cursor: "pointer", border: "none" }}>
          ...
          {/* Units list */}
          {loadingUnits ? (
            <div className="text-center py-6" style={{ color: "var(--text-faint)", fontSize: 13 }}>جاري التحميل...</div>
          ) : units.length === 0 ? (
...
export default function ProjectsPage() {
  ...
  if (missingTable) return (
    <div dir="rtl" style={{ maxWidth: 520, margin: "60px auto", textAlign: "center" }}>
      ...
      <h2 style={{ fontSize: 18, fontWeight: 700, color: "var(--text-strong)", marginBottom: 12 }}>يلزم تفعيل جدول المشاريع</h2>
      <p style={{ fontSize: 13, color: "var(--text-soft)", lineHeight: 1.8 }}>
        شغّل <code style={{ background: "var(--bg-surface-2)", padding: "2px 8px", borderRadius: 6, color: "var(--gold-2)" }}>supabase/006_projects.sql</code> في Supabase → SQL Editor
      </p>
    </div>
  );
    ...
    <div dir="rtl" className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p style={{ color: "var(--text-faint)", fontSize: 13 }}>إدارة المشاريع ووحداتها وتتبع حالة المبيعات</p>
        </div>
      ...
      {showForm && (
        <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface-1)", border: "1px solid rgba(198,145,76,0.18)" }}>
          <div className="flex items-center justify-between mb-5">
            <h3 style={{ fontSize: 13, fontWeight: 700, color: "var(--gold-2)", letterSpacing: 1 }}>مشروع جديد</h3>
            <button onClick={() => setShowForm(false)} style={{ background: "none", border: "none", color: "var(--text-faint)", cursor: "pointer" }}>
              <X size={18} />
            </button>
          </div>
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setShowForm(false)}
                style={{ padding: "12px 20px", borderRadius: 12, background: "var(--bg-surface-2)", color: "var(--text-soft)", fontSize: 13, border: "1px solid var(--gold-bg)", cursor: "pointer" }}>
                إلغاء
              </button>
            </div>
          </div>
      ...
      {projects.length === 0 ? (
        <div className="rounded-2xl py-20 text-center" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg-soft)" }}>
          <Building2 size={40} style={{ color: "var(--gold-bg-hover)", margin: "0 auto 14px", display: "block" }} />
          <p style={{ color: "var(--text-faint)", fontSize: 14, marginBottom: 16 }}>لا توجد مشاريع — أضف أول مشروع</p>
          <button onClick={() => setShowForm(true)}
            style={{ padding: "10px 24px", borderRadius: 12, background: "var(--gold-bg)", border: "1px solid var(--gold-bg-hover)", color: "var(--gold-2)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            + إضافة مشروع
          </button>
        </div>
      ) : (
