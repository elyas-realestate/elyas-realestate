...
async function applyWatermark(file: File, brokerName: string): Promise<Blob> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const positions = [
        {
          x: randBetween(img.width * 0.55, img.width * 0.9),
          y: randBetween(img.height * 0.06, img.height * 0.25),
        },
        {
          x: randBetween(img.width * 0.2, img.width * 0.8),
          y: randBetween(img.height * 0.35, img.height * 0.65),
        },
        {
          x: randBetween(img.width * 0.1, img.width * 0.45),
          y: randBetween(img.height * 0.75, img.height * 0.94),
        },
      ];
...
export default function AddProperty() {
  ...
  async function handleAIDescription() {
    const hasData =
      form.main_category || form.sub_category || form.city || form.district || form.price;
    if (!hasData) {
    ...
    try {
      ...
      const res = await fetch("/api/ai-content", {
        ...
        body: JSON.stringify({
          ...
          mode: "single",
          systemPrompt:
            "أنت خبير تسويق عقاري سعودي متميز. مهمتك كتابة وصف عقاري احترافي وجذاب باللغة العربية الفصحى. الوصف يجب أن يكون:\n- من 3 إلى 5 فقرات\n- يبرز مميزات العقار بشكل جذاب\n- يستخدم مفردات تسويقية مناسبة للسوق السعودي\n- لا يتجاوز 350 كلمة\n- بدون عناوين أو نقاط — نص متدفق فقط",
  ...
  async function handleSubmit(e: any) {
    ...
    if (!form.offer_type) {
      setError("يرجى اختيار نوع العرض (بيع / إيجار / ...)");
      return;
    }
    if (!form.main_category) {
  ...
  if (limitBlocked) {
    return (
      <div dir="rtl" style={{ maxWidth: 480, margin: "80px auto", textAlign: "center" }}>
        ...
        <h2
          style={{ fontSize: 18, fontWeight: 700, color: "var(--text-strong)", marginBottom: 10 }}
        >
          تجاوزت حد خطتك
        </h2>
        <p style={{ fontSize: 14, color: "var(--text-soft)", lineHeight: 1.7, marginBottom: 24 }}>
          {limitBlocked}
        </p>
        <Link
  ...
  return (
    <div dir="rtl">
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="mb-1 text-2xl font-bold">إضافة عقار جديد</h2>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl space-y-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                className={inp}
                placeholder="مثال: فيلا فاخرة في حي النرجس — 5 غرف"
              />
            </div>
        ...
        <Section title="الموقع">
          <div className="space-y-4">
            ...
            <MapsLinkInput
              ...
              onClear={() =>
                setForm((f) => ({ ...f, latitude: null, longitude: null, location_url: "" }))
              }
            />
          </div>
        </Section>
            ...
            <div>
              ...
              >
                ...
                <span style={{ color: "var(--text-faint)", fontWeight: 400, fontSize: 10 }}>
                  اختياري — يبدأ بـ 71 أو 72 ويكون 10 أرقام
                </span>
              </label>
              <div style={{ position: "relative" }}>
                <input
                  ...
                  onChange={(e) =>
                    set("ad_license_number", e.target.value.replace(/\D/g, "").slice(0, 10))
                  }
                  className={inp}
              ...
              {licSt === "ok" && (
                <p style={{ fontSize: 11, color: "var(--success-2)", marginTop: 5 }}>
                  رقم الترخيص صحيح
                </p>
              )}
            </div>
                ...
                {uploadedImages.map((url, i) => (
                  <div
                    ...
                    style={{
                      ...
                      border: "2px solid " + (i === 0 ? "var(--gold-2)" : "transparent"),
                    }}
                  >
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="submit"
            disabled={loading}
            className="flex items-center gap-2 rounded-xl px-8 py-3.5 font-bold text-[var(--bg-page)] transition disabled:opacity-50"
            style={{
              background: loading
                ? "rgba(198,145,76,0.4)"
