...
const PROVIDERS = [
  { id: "openai",    name: "OpenAI (ChatGPT)",    envKey: "OPENAI_API_KEY",    color: "var(--success)", models: ["gpt-4o", "gpt-4o-mini", "gpt-3.5-turbo"] },
  { id: "anthropic", name: "Anthropic (Claude)",   envKey: "ANTHROPIC_API_KEY", color: "var(--purple-ai)", models: ["claude-sonnet-4-6", "claude-opus-4-7", "claude-haiku-4-5-20251001"] },
  { id: "google",    name: "Google (Gemini)",      envKey: "GOOGLE_API_KEY",    color: "var(--info)", models: ["gemini-2.5-flash", "gemini-2.5-pro", "gemini-2.0-flash"] },
  { id: "groq",      name: "Groq (سريع مجاناً)",  envKey: "GROQ_API_KEY",      color: "#F97316", models: ["llama-3.3-70b-versatile", "mixtral-8x7b-32768"] },
  { id: "deepseek",  name: "DeepSeek",             envKey: "DEEPSEEK_API_KEY",  color: "var(--info-2)", models: ["deepseek-chat", "deepseek-reasoner"] },
  { id: "xai",       name: "xAI (Grok)",           envKey: "XAI_API_KEY",       color: "#EC4899", models: ["grok-3", "grok-3-mini"] },
  { id: "manus",     name: "Manus",                envKey: "MANUS_API_KEY",     color: "var(--warning)", models: ["manus-1"] },
];
  ...
  if (missingTable) return (
    <div dir="rtl" style={{ maxWidth: 520, margin: "60px auto", textAlign: "center" }}>
      <div style={{ width: 64, height: 64, borderRadius: 18, background: "var(--gold-bg-soft)", border: "1px solid var(--gold-bg-hover)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
        <Brain size={28} style={{ color: "var(--gold-2)" }} />
      </div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: "var(--text-strong)", marginBottom: 12 }}>يلزم تفعيل مركز الذكاء الاصطناعي</h2>
      <p style={{ fontSize: 13, color: "var(--text-soft)", lineHeight: 1.8 }}>
        شغّل <code style={{ background: "var(--bg-surface-2)", padding: "2px 8px", borderRadius: 6, color: "var(--gold-2)" }}>supabase/012_ai_config.sql</code> في Supabase → SQL Editor
      </p>
    </div>
  );
  ...
  return (
    <div dir="rtl" className="space-y-6">
      ...
      <div>
        ...
        <p style={{ color: "var(--text-faint)", fontSize: 13 }}>خصّص توجيهات AI وربط المزودين وبناء قاعدة المعرفة</p>
      </div>
      ...
      {tab === "prompts" && (
        <div className="space-y-5">
          <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg)" }}>
            <h3 className="font-bold mb-4" style={{ fontSize: 14, color: "var(--gold-2)" }}>
              <MessageSquare size={14} style={{ display: "inline", marginLeft: 6, verticalAlign: "middle" }} />
              التوجيه الرئيسي (System Prompt)
            </h3>
            <p style={{ fontSize: 12, color: "var(--text-faint)", marginBottom: 12, lineHeight: 1.7 }}>
              هذا التوجيه سيُطبق على <strong style={{ color: "var(--gold-2)" }}>جميع</strong> نماذج الـ AI (ChatGPT, Claude, Gemini, Manus). اكتب شخصية وأسلوب المساعد المطلوب.
            </p>
            ...
            <p style={{ fontSize: 10, color: "var(--border-1)", marginTop: 6 }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {/* Personality */}
            <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg)" }}>
              <h3 className="font-bold mb-4" style={{ fontSize: 14, color: "var(--gold-2)" }}>الشخصية</h3>
              <div className="space-y-2">
                {PERSONALITIES.map(p => (
                  <button key={p.id} onClick={() => setConfig(c => ({ ...c, personality: p.id }))}
                    ...
                    style={{
                      background: config.personality === p.id ? "var(--gold-bg-soft)" : "var(--bg-surface-2)",
                      border: "1px solid " + (config.personality === p.id ? "rgba(198,145,76,0.25)" : "rgba(90,90,98,0.15)"),
                      color: config.personality === p.id ? "var(--gold-2)" : "var(--text-soft)",
                      cursor: "pointer", textAlign: "right",
                    }}>
                    <span className="font-semibold">{p.label}</span>
                    <span style={{ fontSize: 11, color: "var(--text-faint)" }}>{p.desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Language + Advanced */}
            <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg)" }}>
              <h3 className="font-bold mb-4" style={{ fontSize: 14, color: "var(--gold-2)" }}>إعدادات متقدمة</h3>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex justify-between" style={{ fontSize: 10, color: "var(--text-faint)" }}>
      ...
      {tab === "providers" && (
        <div className="space-y-5">
          <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg)" }}>
            <h3 className="font-bold mb-2" style={{ fontSize: 14, color: "var(--gold-2)" }}>
              <Key size={14} style={{ display: "inline", marginLeft: 6, verticalAlign: "middle" }} />
              المزود الافتراضي
            </h3>
            <p style={{ fontSize: 12, color: "var(--text-faint)", marginBottom: 16 }}>اختر المزود الافتراضي لجميع عمليات AI</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {PROVIDERS.map(p => (
                <button key={p.id} onClick={() => setConfig(c => ({ ...c, default_provider: p.id }))}
                  className="rounded-xl p-4 text-center transition"
                  style={{
                    background: config.default_provider === p.id ? p.color + "12" : "var(--bg-surface-2)",
                  ...
                  <p style={{ fontSize: 12, fontWeight: 700, color: config.default_provider === p.id ? p.color : "var(--text-soft)" }}>{p.name}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {PROVIDERS.map(p => (
              <div key={p.id} className="rounded-2xl p-5" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg)" }}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-on-dark)" }}>{p.name}</span>
                  </div>
                ...
                <p style={{ fontSize: 12, color: "var(--text-faint)", lineHeight: 1.7 }}>
                  أضف <code style={{ background: "var(--bg-surface-2)", padding: "1px 6px", borderRadius: 4, color: p.color, fontSize: 11 }}>{p.envKey}</code> في ملف <code style={{ background: "var(--bg-surface-2)", padding: "1px 6px", borderRadius: 4, color: "var(--gold-2)", fontSize: 11 }}>.env.local</code>
                </p>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl p-6" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--gold-bg)" }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p style={{ fontSize: 12, color: "var(--text-faint)" }}>المعلومات التي يتعلمها AI ويستخدمها في ردوده</p>
              </div>
              <button onClick={() => setShowAddKB(v => !v)}
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition"
                style={{ background: "linear-gradient(135deg,var(--gold-2),var(--gold-3))", color: "var(--bg-page)", cursor: "pointer", border: "none" }}>
            ...
            {/* Add KB Form */}
            {showAddKB && (
              <div className="rounded-xl p-4 mb-4" style={{ background: "var(--bg-surface-2)", border: "1px solid var(--gold-bg-hover)" }}>
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setShowAddKB(false)} style={{ padding: "10px 16px", borderRadius: 12, background: "var(--bg-surface-2)", color: "var(--text-soft)", fontSize: 12, border: "1px solid var(--gold-bg)", cursor: "pointer" }}>
                      إلغاء
                    </button>
                  </div>
                </div>
            ...
            {knowledge.length === 0 ? (
              <div className="text-center py-10">
                <BookOpen size={32} style={{ color: "var(--border-1)", margin: "0 auto 10px" }} />
                <p style={{ color: "var(--text-faint)", fontSize: 13 }}>لا توجد معرفة مضافة بعد</p>
                <p style={{ color: "var(--border-1)", fontSize: 11, marginTop: 4 }}>أضف معلومات عن السوق والأحياء والأسعار ليستخدمها AI</p>
              </div>
            ) : (
              <div className="space-y-2">
                {knowledge.map(kb => (
                  <div key={kb.id} className="flex items-start gap-3 p-4 rounded-xl transition"
                    style={{ background: "var(--bg-surface-2)", border: "1px solid rgba(90,90,98,0.1)" }}>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 style={{ fontSize: 13, fontWeight: 700, color: "var(--text-on-dark)" }}>{kb.title}</h4>
                        <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 4, background: "var(--gold-bg-soft)", color: "var(--gold-2)" }}>{kb.category}</span>
                      </div>
                      <p style={{ fontSize: 12, color: "var(--text-faint)", lineHeight: 1.6 }} className="line-clamp-2">{kb.content}</p>
                    </div>
                    <button onClick={() => deleteKnowledge(kb.id)} style={{ background: "none", border: "none", color: "var(--danger)", cursor: "pointer", flexShrink: 0, padding: 4 }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
