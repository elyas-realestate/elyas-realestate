    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
        <div style={{
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 3 }}>الرئيس التنفيذي</div>
          <div style={{ fontSize: 16, fontWeight: 800, color: "var(--text-primary)" }}>{brokerName}</div>
        </div>
      </div>

      {/* Connecting line */}
      <div style={{
        width: 2, height: 30, background: "linear-gradient(to bottom, var(--gold-2), transparent)",
        margin: "0 auto",
      }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))", gap: 10, marginBottom: 22 }}>
        ...
        <SummaryStat label="توجيهات نشطة" value={totalDirectives} icon={Sparkles} color="var(--purple-ai)" />
        <SummaryStat label="اقتراحات تنتظر مراجعتك" value={totalPending} icon={Bot} color={totalPending > 0 ? "var(--gold-1)" : "var(--text-ghost)"} />
      ...
      {loading ? (
        <div style={{ display: "flex", justifyContent: "center", padding: 60 }}>
          <Loader2 size={26} style={{ color: "var(--purple-ai)", animation: "spin 1s linear infinite" }} />
        </div>
      ) : managers.length === 0 ? (
        <div style={{ background: "var(--bg-deep)", border: "1px solid var(--overlay-mid)", borderRadius: 12, padding: 40, textAlign: "center", color: "var(--text-disabled)" }}>
          <Network size={32} style={{ color: "var(--border-1)", marginBottom: 10 }} />
          <div style={{ fontSize: 14, color: "var(--text-muted)" }}>لا يوجد مدراء — هل تم تشغيل migration 031؟</div>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(290px, 1fr))", gap: 14 }}>
          {managers.map(m => {
            const dept = DEPARTMENT_META[m.department] || { label: m.department, color: "var(--text-muted)", bg: "rgba(161,161,170,0.10)", icon: Bot };
            const Icon = dept.icon;
            return (
              <Link key={m.manager_id} href={`/dashboard/organization/manager/${m.manager_id}`}
                style={{
                  background: "var(--bg-deep)", border: `1px solid ${dept.color}33`,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-on-dark)", lineHeight: 1.3 }}>{m.manager_name}</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  ...
                  <Mini label="اقتراحات" value={m.pending_suggestions}  color={m.pending_suggestions > 0 ? "var(--gold-1)" : "var(--text-disabled)"} highlight={m.pending_suggestions > 0} />
                  <Mini label="معرفة"    value={m.kb_items}             color="var(--info)" />
                </div>

                {/* Status badge */}
                {!m.manager_enabled && (
                  <div style={{ fontSize: 10, color: "var(--danger)", background: "rgba(239,68,68,0.06)", padding: "4px 8px", borderRadius: 5, textAlign: "center" }}>
                    ⚠ معطّل
                  </div>
                )}
              </Link>
      ...
      {/* Tip card */}
      <div style={{ marginTop: 22, padding: 14, background: "rgba(96,165,250,0.05)", border: "1px solid rgba(96,165,250,0.15)", borderRadius: 11, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.8 }}>
...
function SummaryStat({ label, value, icon: Icon, color }: { label: string; value: number; icon: typeof Network; color: string }) {
  return (
    <div style={{ background: "var(--bg-deep)", border: "1px solid var(--overlay-soft)", borderRadius: 11, padding: "12px 14px" }}>
      ...
      <div style={{ fontSize: 20, fontWeight: 800, color: "var(--text-primary)", lineHeight: 1.1 }}>{value}</div>
...
function Mini({ label, value, color, highlight }: { label: string; value: number; color: string; highlight?: boolean }) {
  return (
    <div style={{
      padding: "8px 10px", background: highlight ? `${color}15` : "var(--bg-surface-2)",
