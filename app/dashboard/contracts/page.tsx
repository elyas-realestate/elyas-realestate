  ...
  async function load() {
    ...
    try {
      const { data, error: e } = await supabase
        .from("e_contracts")
        .select(
          "id, contract_number, title, category, status, amount, start_date, end_date, created_at, party_first, party_second"
        )
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td
                    colSpan={7}
                    style={{ padding: 40, textAlign: "center", color: "var(--text-disabled)" }}
                  >
                    {contracts.length === 0 ? (
                      <div>
                        <FileSignature
                          size={32}
                          style={{ color: "var(--border-1)", marginBottom: 10 }}
                        />
                        <div style={{ fontSize: 14, color: "var(--text-muted)", marginBottom: 4 }}>
                          لا توجد عقود بعد
                        </div>
                        <div style={{ fontSize: 12, marginBottom: 14 }}>
                          ابدأ بعقد جديد من قالب جاهز
                        </div>
                        <Link
                          href="/dashboard/contracts/new"
                          style={{
                ...
                filtered.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        style={{ padding: "11px 14px", color: "var(--text-muted)", fontSize: 12 }}
                      >
