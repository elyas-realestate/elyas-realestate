...
const statusConfig: Record<string, { color: string; bg: string }> = {
  ...
  "قيد التنفيذ": { color: "text-[#C9A84C]", bg: "bg-[rgba(201,168,76,0.1)]" },
  ...
  return (
    <div dir="rtl">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { resetForm(); setShowForm(true); }} className="flex items-center gap-2 px-5 py-3 rounded-xl font-bold transition text-[#0A0A0C]" style={{ background:'linear-gradient(135deg, #C9A84C, #A68A3A)' }}>
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        {[
          { label: "الإجمالي", value: stats.total, color: "#C9A84C", icon: CheckSquare },
          { label: "جديدة", value: stats.new, color: "#60A5FA", icon: Circle },
          { label: "قيد التنفيذ", value: stats.inProgress, color: "#C9A84C", icon: Clock },
          ...
          <div key={i} className="rounded-xl p-4 flex items-center gap-3" style={{ background:'#16161A', border:'1px solid rgba(201,168,76,0.12)' }}>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <div className="relative flex-1 max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث..." className="w-full rounded-lg pr-10 pl-4 py-2.5 text-sm focus:outline-none" style={{ background:'#16161A', border:'1px solid rgba(201,168,76,0.12)', color:'#F5F5F5' }} />
        </div>
        <select value={filterPriority} onChange={e => setFilterPriority(e.target.value)} className="rounded-lg px-3 py-2.5 text-sm focus:outline-none" style={{ background:'#16161A', border:'1px solid rgba(201,168,76,0.12)', color:'#F5F5F5' }}>
          <option value="all">كل الأولويات</option>
          {priorities.map(p => <option key={p} value={p}>{p}</option>)}
        </select>
        <select value={filterType} onChange={e => setFilterType(e.target.value)} className="rounded-lg px-3 py-2.5 text-sm focus:outline-none" style={{ background:'#16161A', border:'1px solid rgba(201,168,76,0.12)', color:'#F5F5F5' }}>
          <option value="all">كل الأنواع</option>
          {taskTypes.map(t => <option key={t} value={t}>{t}</option>)}
        </select>
        <div className="flex rounded-lg overflow-hidden mr-auto" style={{ border:'1px solid rgba(201,168,76,0.12)' }}>
          {([["list","قائمة",List],["kanban","كانبان",LayoutGrid],["calendar","تقويم",Calendar]] as [string,string,any][]).map(([id,label,Icon]) => (
            <button key={id} onClick={() => setView(id as any)} className="flex items-center gap-1.5 px-3 py-2 text-xs transition" style={{ background: view === id ? 'rgba(201,168,76,0.15)' : '#16161A', color: view === id ? '#C9A84C' : '#5A5A62' }}>
              <Icon size={14} />{label}
            </button>
          ))}
        </div>
      ...
      {/* ═══ LIST VIEW ═══ */}
      {view === "list" && (
        <div className="rounded-xl overflow-hidden" style={{ background:'#16161A', border:'1px solid rgba(201,168,76,0.12)' }}>
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={task.id} className="transition" style={{ borderBottom:'1px solid rgba(201,168,76,0.05)' }}>
      ...
      {view === "kanban" && (
        <div className="grid grid-cols-4 gap-4" style={{ minHeight:400 }}>
          {statuses.map(status => {
            ...
            return (
              <div key={status} className="rounded-xl" style={{ background:'#111114', border:'1px solid rgba(201,168,76,0.08)', padding:12 }}>
                <div className="flex items-center justify-between mb-3 px-1">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full" style={{ background: conf?.color.includes('blue') ? '#60A5FA' : conf?.color.includes('C9A84C') ? '#C9A84C' : conf?.color.includes('purple') ? '#C084FC' : '#4ADE80' }}></div>
                    <span className="text-sm font-bold" style={{ color:'#F5F5F5' }}>{status}</span>
                  </div>
                  <span className="text-xs px-2 py-0.5 rounded-full" style={{ background:'rgba(201,168,76,0.08)', color:'#5A5A62' }}>{statusTasks.length}</span>
                </div>
                <div className="space-y-2">
                  {statusTasks.map(task => (
                    <div key={task.id} className="rounded-xl p-3 transition cursor-pointer" style={{ background:'#16161A', border:'1px solid rgba(201,168,76,0.08)' }} onClick={() => openEdit(task)}>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {task.task_type && <span className="text-xs px-1.5 py-0.5 rounded" style={{ background:'rgba(201,168,76,0.06)', color:'#9A9AA0' }}>{task.task_type}</span>}
                  ...
                  <button onClick={() => { resetForm(); setForm(f => ({...f, status})); setShowForm(true); }} className="w-full py-2 rounded-lg text-xs transition" style={{ color:'#5A5A62', border:'1px dashed rgba(201,168,76,0.12)' }}>+ إضافة مهمة</button>
                </div>
              </div>
            );
          })}
      ...
      {view === "calendar" && (
        <div className="rounded-xl overflow-hidden" style={{ background:'#16161A', border:'1px solid rgba(201,168,76,0.12)' }}>
          <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom:'1px solid rgba(201,168,76,0.08)' }}>
          ...
          <div className="grid grid-cols-7" style={{ borderBottom:'1px solid rgba(201,168,76,0.08)' }}>
          ...
          <div className="grid grid-cols-7">
            {getMonthDays().map((item, idx) => {
              ...
              return (
                <div key={idx} className="min-h-[90px] p-1.5" style={{ borderLeft:'1px solid rgba(201,168,76,0.05)', borderBottom:'1px solid rgba(201,168,76,0.05)', background: item.current ? 'transparent' : 'rgba(10,10,12,0.5)' }}>
                  <div className={"text-xs font-bold mb-1 w-6 h-6 flex items-center justify-center rounded-full " + (isToday ? "text-[#0A0A0C]" : "")} style={{ background: isToday ? '#C9A84C' : 'transparent', color: isToday ? '#0A0A0C' : item.current ? '#9A9AA0' : '#3A3A42' }}>{item.day}</div>
                  {dayTasks.slice(0, 3).map(t => (
                    <div key={t.id} onClick={() => openEdit(t)} className="text-xs rounded px-1 py-0.5 mb-0.5 truncate cursor-pointer" style={{ background: priorityConfig[t.priority]?.bg.replace('bg-','').replace('[','').replace(']','') || 'rgba(201,168,76,0.06)', color: t.status === "مكتملة" ? '#5A5A62' : '#F5F5F5' }}>{t.title}</div>
                  ))}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => { setShowForm(false); resetForm(); }}>
          <div className="rounded-2xl max-w-lg w-full p-6" style={{ background:'#16161A', border:'1px solid rgba(201,168,76,0.15)' }} dir="rtl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full rounded-lg px-4 py-3 text-sm focus:outline-none" style={{ background:'#1C1C22', border:'1px solid rgba(201,168,76,0.15)', color:'#F5F5F5' }} placeholder="مثال: متابعة عميل حي النرجس" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm mb-2" style={{ color:'#9A9AA0' }}>النوع</label>
                  <select value={form.task_type} onChange={e => setForm({...form, task_type: e.target.value})} className="w-full rounded-lg px-4 py-3 text-sm focus:outline-none" style={{ background:'#1C1C22', border:'1px solid rgba(201,168,76,0.15)', color:'#F5F5F5' }}>
                ...
                <div>
                  ...
                  <select value={form.priority} onChange={e => setForm({...form, priority: e.target.value})} className="w-full rounded-lg px-4 py-3 text-sm focus:outline-none" style={{ background:'#1C1C22', border:'1px solid rgba(201,168,76,0.15)', color:'#F5F5F5' }}>
                    {priorities.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" value={form.due_date} onChange={e => setForm({...form, due_date: e.target.value})} className="w-full rounded-lg px-4 py-3 text-sm focus:outline-none" style={{ background:'#1C1C22', border:'1px solid rgba(201,168,76,0.15)', color:'#F5F5F5' }} />
                </div>
                <div>
                  <label className="block text-sm mb-2" style={{ color:'#9A9AA0' }}>الحالة</label>
                  <select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="w-full rounded-lg px-4 py-3 text-sm focus:outline-none" style={{ background:'#1C1C22', border:'1px solid rgba(201,168,76,0.15)', color:'#F5F5F5' }}>
                    {statuses.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
              </div>
              <div>
                ...
                <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows={3} className="w-full rounded-lg px-4 py-3 text-sm focus:outline-none" style={{ background:'#1C1C22', border:'1px solid rgba(201,168,76,0.15)', color:'#F5F5F5' }} placeholder="تفاصيل إضافية..." />
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={handleSave} className="flex-1 py-3 rounded-xl font-bold transition flex items-center justify-center gap-2 text-[#0A0A0C]" style={{ background:'linear-gradient(135deg, #C9A84C, #A68A3A)' }}>
