...
export default function Content() {
  ...
  const filtered = items.filter(i => i.title?.includes(search) || i.topic?.includes(search));
      ...
      <main className="p-8">
        {showAdd && (
          <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-8 grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-2">ركيزة المحتوى</label>
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-2">الصيغة</label>
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-2">تاريخ النشر</label>
            ...
            <div className="col-span-2">
              <label className="block text-sm text-gray-400 mb-2">نص المحتوى</label>
        ...
        <div className="relative mb-6 max-w-md">
          ...
          <input type="text" placeholder="ابحث عن محتوى..." value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-gray-900 border border-gray-800 rounded-lg pr-10 pl-4 py-3 focus:outline-none focus:border-blue-500" />
        </div>
        {filtered.length === 0 ? (
          <p className="text-gray-400 text-center py-20">لا يوجد محتوى بعد</p>
        ) : (
              ...
              <div key={item.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-blue-500 transition">
                ...
                {item.main_text && (
                  <div className="relative">
                    ...
                    <button onClick={() => copyText(item.id, item.main_text)} className="absolute top-1 left-1 text-gray-500 hover:text-white transition">
                      {copied === item.id ? <span className="text-green-400 text-xs">نُسخ</span> : <Copy size={14} />}
                    </button>
                  </div>
                )}
