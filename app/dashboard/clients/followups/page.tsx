  ...
  async function load() {
    ...
    try {
      ...
      const clientIds = Array.from(
        new Set(queue.map((i) => i.client_id).filter(Boolean) as string[])
      );
    ...
    <div>
      ...
      >
        <div>
          ...
          >
            <MessageCircle size={20} style={{ color: "var(--success-2)" }} /> رسائل المتابعة (موظف
            المتابعة)
          </h1>
                    ...
                    {isEditing ? (
                      <>
                        <button
                          onClick={saveEdit}
                          disabled={isBusy}
                          style={btnSm("var(--success)", "rgba(74,222,128,0.1)")}
                        >
                          <Check size={11} /> حفظ
                        </button>
                        <button
                      ...
                      <>
                        <button
                          ...
                          style={btnSm("var(--purple-ai)", "rgba(124,58,237,0.08)")}
                        >
                          <Edit3 size={11} /> تعديل
                        </button>
                        <button
                          ...
                          disabled={isBusy}
                          style={btnSm("var(--text-ghost)", "rgba(113,113,122,0.08)")}
                        >
                          <X size={11} /> تخطٍّ
                        </button>
                        <button
                          onClick={() => send(item)}
                          disabled={isBusy || !item.client?.phone}
                          style={btnSm("var(--success-2)", "rgba(52,211,153,0.12)")}
                        >
                          {isBusy ? (
