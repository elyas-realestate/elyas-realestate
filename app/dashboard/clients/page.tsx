    ...
    <div  dir="rtl">
      ...
      <main className="p-8">
        {showAdd && (
          <form onSubmit={handleSubmit} className="bg-[#16161A] border border-[rgba(198,145,76,0.12)] rounded-xl p-6 mb-8 grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="full_name" value={form.full_name} onChange={handleChange} required className="w-full bg-[#1C1C22] border border-[rgba(198,145,76,0.15)] rounded-lg px-4 py-2 focus:outline-none focus:border-[#C6914C]" />
            </div>
            <div>
              <label className="block text-sm text-[#9A9AA0] mb-2">رقم الجوال</label>
              <input name="phone" value={form.phone} onChange={handleChange} className="w-full bg-[#1C1C22] border border-[rgba(198,145,76,0.15)] rounded-lg px-4 py-2 focus:outline-none focus:border-[#C6914C]" />
            </div>
            <div>
              <label className="block text-sm text-[#9A9AA0] mb-2">الفئة *</label>
              <select name="category" value={form.category} onChange={handleChange} required className="w-full bg-[#1C1C22] border border-[rgba(198,145,76,0.15)] rounded-lg px-4 py-2 focus:outline-none focus:border-[#C6914C]">
            ...
            <div>
              ...
              <input name="city" value={form.city} onChange={handleChange} className="w-full bg-[#1C1C22] border border-[rgba(198,145,76,0.15)] rounded-lg px-4 py-2 focus:outline-none focus:border-[#C6914C]" />
            </div>
            <div className="col-span-2">
              <label className="block text-sm text-[#9A9AA0] mb-2">ملاحظات</label>
              <textarea name="notes" value={form.notes} onChange={handleChange} rows={2} className="w-full bg-[#1C1C22] border border-[rgba(198,145,76,0.15)] rounded-lg px-4 py-2 focus:outline-none focus:border-[#C6914C]" />
            </div>
        ...
        <div className="relative mb-6 max-w-md">
          ...
          <input type="text" placeholder="ابحث باسم العميل أو الجوال..." value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-[#16161A] border border-[rgba(198,145,76,0.12)] rounded-lg pr-10 pl-4 py-3 focus:outline-none focus:border-[#C6914C]" />
        </div>
        {filtered.length === 0 ? (
