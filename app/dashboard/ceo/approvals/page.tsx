...
export default function CEOApprovalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok) {
        throw new Error(
          j.error ||
            `الخادم رجع ${r.status}${text ? `: ${text.slice(0, 120)}` : " (استجابة فارغة)"}`
        );
      }
  ...
  async function decide(id: string, decision: "approved" | "rejected" | "modified") {
    ...
    try {
      ...
      if (!r.ok)
        throw new Error(
          j.error || `الخادم رجع ${r.status}${text ? `: ${text.slice(0, 120)}` : ""}`
        );
      toast.success(
        decision === "approved"
          ? "تم الاعتماد"
    ...
    <div style={{ direction: "rtl", padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Link
            href="/dashboard/ceo"
            style={{ color: "#9CA3AF", display: "flex", alignItems: "center", gap: 4 }}
          >
            <ChevronLeft size={18} />
      ...
      {loading ? (
        ...
        >
          ...
          <p style={{ fontSize: 14, margin: 0 }}>
            {filter === "pending"
              ? "كل شيء تحت السيطرة. لا توجد طلبات معلَّقة."
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {items.map((item) => {
            ...
            const dept = item.employee?.department
              ? DEPARTMENT_META[item.employee.department as keyof typeof DEPARTMENT_META]
              : null;
            ...
            const verdictReason =
              typeof action.verdict_reason === "string" ? action.verdict_reason : "";
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      ...
                      {tags.length > 0 && <span>التصنيفات: {tags.join("، ")}</span>}
                    </div>
                  </div>
                </div>
                ...
                {isExpanded && (
                  ...
                  >
                    ...
                    {action.payload != null &&
                      ...
                      Object.keys(action.payload as Record<string, unknown>).length > 0 && (
                    ...
                    {item.status !== "pending" && item.ceo_decision && (
                      ...
                      >
                        ...
                        >
                          قرارك (
                          {item.decided_at
                            ? new Date(item.decided_at).toLocaleString("ar-SA", {
                                dateStyle: "short",
                                timeStyle: "short",
                              })
                            : ""}
                        ...
                        <div style={{ fontSize: 13, color: "#E5E7EB" }}>{item.ceo_decision}</div>
                      </div>
                    )}
                    ...
                    {item.status === "pending" && (
                      <>
                        <textarea
                          ...
                          onChange={(e) =>
                            setNoteDraft({ ...noteDraft, [item.id]: e.target.value })
                          }
