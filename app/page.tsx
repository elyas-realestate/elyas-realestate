  ...
  return (
    <div dir="rtl" className="bg-[#0A0A0C] text-[#F5F5F5] min-h-screen" style={{ fontFamily: "'Tajawal', sans-serif" }}>
      <style>{`
        ...
        .border-accent { border-color: rgba(201,168,76,0.12); }
        .border-accent-hover:hover { border-color: rgba(201,168,76,0.3); }
        .glass { background: rgba(22,22,26,0.85); backdrop-filter: blur(20px); }
        .card { background: #16161A; border: 1px solid rgba(201,168,76,0.12); border-radius: 16px; }
        .card:hover { border-color: rgba(201,168,76,0.3); transform: translateY(-4px); }
        .hero-bg { background: linear-gradient(180deg, rgba(10,10,12,0.3) 0%, rgba(10,10,12,0.1) 40%, rgba(10,10,12,0.85) 85%, rgba(10,10,12,1) 100%), url('https://images.unsplash.com/photo-1578469550956-0e16b69c6a3d?w=1920&q=80') center/cover no-repeat; }
      ...
      {/* ═══════ NAVBAR ═══════ */}
      <nav className="fixed top-0 right-0 left-0 z-50 glass border-b border-accent" style={{ height: 72, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 48px' }}>
        ...
        <div className="flex items-center gap-9">
          <Link href="/" className="text-[#9A9AA0] hover:text-white no-underline text-sm font-medium">الرئيسية</Link>
          <Link href="/properties" className="text-[#9A9AA0] hover:text-white no-underline text-sm font-medium">العقارات</Link>
          <a href="#services" className="text-[#9A9AA0] hover:text-white no-underline text-sm font-medium">الخدمات</a>
          <a href="#why" className="text-[#9A9AA0] hover:text-white no-underline text-sm font-medium">لماذا {site_name.split(' ')[0]}</a>
          <a href="#contact" className="accent-bg text-[#0A0A0C] no-underline font-bold text-sm" style={{ padding:'10px 24px', borderRadius:10 }}>تواصل معي</a>
        </div>
      </nav>

      {/* ═══════ HERO ═══════ */}
      <section className="hero-bg relative flex items-center justify-center" style={{ minHeight:'100vh' }}>
        ...
        <div className="relative z-10 text-center fade-up" style={{ maxWidth:860, padding:'0 24px' }}>
          <div className="fade-up inline-flex items-center gap-2 accent text-[13px] font-medium" style={{ background:'rgba(201,168,76,0.1)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:100, padding:'8px 20px', marginBottom:32 }}>
            <span style={{ width:6, height:6, background:'#C9A84C', borderRadius:'50%' }}></span>
            {hero_badge}
          </div>
          <h1 className="font-kufi font-black fade-up fade-up-d1" style={{ fontSize:'clamp(2.4rem, 5.5vw, 4.2rem)', lineHeight:1.25, marginBottom:20 }}>
          ...
          <p className="fade-up fade-up-d2" style={{ fontSize:'clamp(1rem, 2vw, 1.2rem)', color:'#9A9AA0', lineHeight:1.8, maxWidth:600, margin:'0 auto 40px' }}>{hero_subtitle}</p>

          {/* Search Bar */}
          <div className="fade-up fade-up-d3 card flex items-center gap-2" style={{ maxWidth:720, margin:'0 auto', padding:8 }}>
            <div className="flex-1 flex flex-col" style={{ padding:'8px 16px', borderLeft:'1px solid rgba(201,168,76,0.12)' }}>
              <span style={{ fontSize:11, color:'#5A5A62', fontWeight:600, textTransform:'uppercase', letterSpacing:0.5, marginBottom:4 }}>نوع العقار</span>
              <span style={{ fontSize:14, color:'#F5F5F5', fontWeight:500 }}>شقة، فيلا، أرض</span>
            </div>
            <div className="flex-1 flex flex-col" style={{ padding:'8px 16px', borderLeft:'1px solid rgba(201,168,76,0.12)' }}>
              <span style={{ fontSize:11, color:'#5A5A62', fontWeight:600, textTransform:'uppercase', letterSpacing:0.5, marginBottom:4 }}>المنطقة</span>
            ...
            <div className="flex-1 flex flex-col" style={{ padding:'8px 16px' }}>
              <span style={{ fontSize:11, color:'#5A5A62', fontWeight:600, textTransform:'uppercase', letterSpacing:0.5, marginBottom:4 }}>العملية</span>
              <span style={{ fontSize:14, color:'#F5F5F5', fontWeight:500 }}>بيع، إيجار</span>
            </div>
            <Link href="/properties" className="accent-bg no-underline text-[#0A0A0C] font-bold text-[15px] flex items-center gap-2" style={{ padding:'14px 28px', borderRadius:10, whiteSpace:'nowrap' }}>
              🔍 بحث
            </Link>
          </div>
        </div>
      ...
      <section id="why" style={{ padding:'100px 48px', background:'#111114' }}>
        <div className="text-center" style={{ marginBottom:64 }}>
          <div className="accent inline-flex items-center gap-2 text-[13px] font-semibold" style={{ textTransform:'uppercase', letterSpacing:2, marginBottom:16 }}>
            <span style={{ width:32, height:1, background:'#C9A84C', opacity:0.4 }}></span>القيمة المضافة<span style={{ width:32, height:1, background:'#C9A84C', opacity:0.4 }}></span>
          </div>
          ...
          <p style={{ fontSize:16, color:'#9A9AA0', maxWidth:560, margin:'0 auto', lineHeight:1.8 }}>لأن الفرق الحقيقي مو بالإعلان — بل بالفهم، والتنظيم، والمتابعة حتى تتم الصفقة</p>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:24, maxWidth:1200, margin:'0 auto' }}>
          {(why_cards as any[]).map((card: any, i: number) => (
            <div key={i} className="card" style={{ padding:'40px 32px', transition:'all 0.4s cubic-bezier(0.16, 1, 0.3, 1)', position:'relative', overflow:'hidden' }}>
      ...
      {properties.length > 0 && (
        <section style={{ padding:'100px 48px' }}>
          <div className="text-center" style={{ marginBottom:64 }}>
            <div className="accent inline-flex items-center gap-2 text-[13px] font-semibold" style={{ textTransform:'uppercase', letterSpacing:2, marginBottom:16 }}>
              <span style={{ width:32, height:1, background:'#C9A84C', opacity:0.4 }}></span>عقارات مختارة<span style={{ width:32, height:1, background:'#C9A84C', opacity:0.4 }}></span>
            </div>
            <h2 className="font-kufi font-extrabold" style={{ fontSize:'clamp(1.8rem, 3.5vw, 2.6rem)', lineHeight:1.3, marginBottom:16 }}>لا نعرض كل شي — فقط اللي يستاهل</h2>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:24, maxWidth:1200, margin:'0 auto' }}>
            {properties.map((p: any) => (
              <Link href={"/properties/" + p.id} key={p.id} className="card no-underline text-white" style={{ overflow:'hidden', transition:'all 0.4s cubic-bezier(0.16, 1, 0.3, 1)', cursor:'pointer' }}>
                <div style={{ height:220, overflow:'hidden', position:'relative', background:'#1C1C22' }}>
                  {p.images?.[0] ? (
                    <img src={p.images[0]} alt={p.title} style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                  ) : (
                    <div className="flex items-center justify-center h-full text-[#5A5A62] text-4xl">🏠</div>
                  )}
                ...
                <div style={{ padding:24 }}>
                  ...
                  <p style={{ fontSize:13.5, color:'#9A9AA0', marginBottom:18, display:'flex', alignItems:'center', gap:6 }}>
                    📍 {p.district}، {p.city}
                  </p>
                  ...
                  {p.price && (
                    <div className="font-kufi accent" style={{ fontSize:20, fontWeight:800 }}>
                      {Number(p.price).toLocaleString()} <span style={{ fontSize:13, color:'#5A5A62', fontWeight:500 }}>ريال</span>
                    </div>
                  )}
                </div>
              </Link>
          ...
          <div className="text-center" style={{ marginTop:40 }}>
            <Link href="/properties" className="no-underline" style={{ color:'#9A9AA0', border:'1px solid rgba(201,168,76,0.12)', padding:'14px 36px', borderRadius:12, fontSize:15, fontWeight:600, display:'inline-block' }}>عرض جميع العقارات</Link>
          </div>
        </section>
      )}
      ...
      <section id="services" style={{ padding:'100px 48px', background:'#111114' }}>
        <div className="text-center" style={{ marginBottom:64 }}>
          <div className="accent inline-flex items-center gap-2 text-[13px] font-semibold" style={{ textTransform:'uppercase', letterSpacing:2, marginBottom:16 }}>
            <span style={{ width:32, height:1, background:'#C9A84C', opacity:0.4 }}></span>خدماتنا<span style={{ width:32, height:1, background:'#C9A84C', opacity:0.4 }}></span>
          </div>
          <h2 className="font-kufi font-extrabold" style={{ fontSize:'clamp(1.8rem, 3.5vw, 2.6rem)', lineHeight:1.3, marginBottom:16 }}>خدمات عقارية متكاملة</h2>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:20, maxWidth:1200, margin:'0 auto' }}>
          {(services as any[]).map((svc: any, i: number) => (
            <div key={i} className="card text-center" style={{ padding:'36px 28px', transition:'all 0.4s cubic-bezier(0.16, 1, 0.3, 1)' }}>
      ...
      <section id="contact" style={{ padding:'100px 48px', textAlign:'center', position:'relative', overflow:'hidden' }}>
        ...
        <div className="card relative" style={{ maxWidth:720, margin:'0 auto', padding:'64px 48px', borderRadius:24 }}>
          ...
          <div className="flex justify-center gap-4 flex-wrap">
            {whatsapp && (
              <a href={"https://wa.me/" + whatsapp} target="_blank" className="accent-bg no-underline text-[#0A0A0C] font-bold flex items-center gap-2" style={{ padding:'14px 36px', borderRadius:12, fontSize:15 }}>
                💬 واتساب
              </a>
            )}
            {phone && (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1.5fr 1fr 1fr 1fr', gap:48, maxWidth:1200, margin:'0 auto 48px' }}>
          <div>
            ...
            <div className="flex gap-3" style={{ marginTop:20 }}>
              {social_x && <a href={social_x} target="_blank" className="flex items-center justify-center no-underline text-[#5A5A62] hover:text-[#C9A84C]" style={{ width:40, height:40, borderRadius:10, border:'1px solid rgba(201,168,76,0.12)', fontSize:16 }}>𝕏</a>}
              {social_instagram && <a href={social_instagram} target="_blank" className="flex items-center justify-center no-underline text-[#5A5A62] hover:text-[#C9A84C]" style={{ width:40, height:40, borderRadius:10, border:'1px solid rgba(201,168,76,0.12)', fontSize:16 }}>📸</a>}
              {social_tiktok && <a href={social_tiktok} target="_blank" className="flex items-center justify-center no-underline text-[#5A5A62] hover:text-[#C9A84C]" style={{ width:40, height:40, borderRadius:10, border:'1px solid rgba(201,168,76,0.12)', fontSize:16 }}>♪</a>}
              {social_linkedin && <a href={social_linkedin} target="_blank" className="flex items-center justify-center no-underline text-[#5A5A62] hover:text-[#C9A84C]" style={{ width:40, height:40, borderRadius:10, border:'1px solid rgba(201,168,76,0.12)', fontSize:16 }}>in</a>}
            </div>
          </div>
          <div>
            <h4 className="font-kufi" style={{ fontSize:15, fontWeight:700, marginBottom:20 }}>روابط سريعة</h4>
            <div className="flex flex-col gap-3">
              <Link href="/" className="no-underline text-[#9A9AA0] hover:text-[#C9A84C] text-sm">الرئيسية</Link>
              <Link href="/properties" className="no-underline text-[#9A9AA0] hover:text-[#C9A84C] text-sm">العقارات</Link>
              <a href="#services" className="no-underline text-[#9A9AA0] hover:text-[#C9A84C] text-sm">الخدمات</a>
              <a href="#why" className="no-underline text-[#9A9AA0] hover:text-[#C9A84C] text-sm">لماذا {site_name.split(' ')[0]}</a>
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-col gap-3">
              {(services as any[]).map((svc: any, i: number) => (
                <span key={i} className="text-[#9A9AA0] text-sm">{svc.title}</span>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-col gap-3">
              {whatsapp && <a href={"https://wa.me/" + whatsapp} className="no-underline text-[#9A9AA0] hover:text-[#C9A84C] text-sm">واتساب</a>}
              {email && <a href={"mailto:" + email} className="no-underline text-[#9A9AA0] hover:text-[#C9A84C] text-sm">{email}</a>}
              {phone && <a href={"tel:" + phone} className="no-underline text-[#9A9AA0] hover:text-[#C9A84C] text-sm">{phone}</a>}
