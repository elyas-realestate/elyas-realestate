...
export default function PropertyDetail() {
  ...
  if (loading) return (
    <div dir="rtl" style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Tajawal', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, color: "#C9A84C" }}>
  ...
  if (!property) return (
    <div dir="rtl" style={{ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Tajawal', sans-serif", textAlign: "center" }}>
      ...
      <Link href="/properties" style={{ background: "linear-gradient(135deg, #C9A84C, #A68A3A)", color: "#0A0A0C", textDecoration: "none", padding: "12px 28px", borderRadius: 12, fontWeight: 700, fontSize: 14, fontFamily: "'Tajawal', sans-serif" }}>العودة للعقارات</Link>
    </div>
  );
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "'Tajawal', sans-serif" }}>
      <style>{`
        ...
        .spec-card:hover { border-color: rgba(201,168,76,0.25) !important; background: rgba(201,168,76,0.04) !important; }
        .action-btn { transition: all 0.25s; }
        .action-btn:hover { opacity: 0.88; transform: translateY(-1px); }
        .thumb-btn:hover { border-color: rgba(201,168,76,0.5) !important; }
      `}</style>

      {/* ═══ BREADCRUMB ═══ */}
      <div style={{ background: "#111114", borderBottom: "1px solid rgba(201,168,76,0.08)", padding: "14px 48px" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 48px" }} className="detail-fade">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 32, alignItems: "start" }}>
          ...
          <div>
            {/* Image Gallery */}
            <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 24, background: "#16161A", border: "1px solid rgba(201,168,76,0.12)" }}>
              {images.length > 0 ? (
                <>
                  <div style={{ position: "relative", height: 440 }}>
                    ...
                    <div style={{ position: "absolute", top: 20, right: 20 }}>
                      <span style={{ background: "linear-gradient(135deg, #C9A84C, #A68A3A)", color: "#0A0A0C", fontSize: 13, fontWeight: 700, padding: "7px 16px", borderRadius: 10 }}>
                  ...
                  {images.length > 1 && (
                    <div style={{ display: "flex", gap: 8, padding: 12, overflowX: "auto" }}>
                      {images.map((img, i) => (
                        <button key={i} onClick={() => setActiveImg(i)} className="thumb-btn" style={{ flexShrink: 0, width: 72, height: 54, borderRadius: 8, overflow: "hidden", border: "2px solid " + (activeImg === i ? "#C9A84C" : "rgba(201,168,76,0.08)"), cursor: "pointer", padding: 0, transition: "border-color 0.25s" }}>
            ...
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                {property.sub_category && (
                  <span style={{ background: "rgba(201,168,76,0.1)", color: "#C9A84C", fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 20, border: "1px solid rgba(201,168,76,0.2)" }}>
                    {property.sub_category}
                  </span>
                )}
                {property.main_category && (
                  <span style={{ background: "#1C1C22", color: "#9A9AA0", fontSize: 12, padding: "5px 12px", borderRadius: 20, border: "1px solid rgba(201,168,76,0.08)" }}>
                    {property.main_category}
                  </span>
                )}
              </div>
            ...
            {specs.length > 0 && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12, marginBottom: 28 }}>
                {specs.map((spec, i) => (
                  <div key={i} className="spec-card" style={{ background: "#16161A", border: "1px solid rgba(201,168,76,0.1)", borderRadius: 14, padding: "18px 16px", textAlign: "center", transition: "all 0.25s" }}>
            ...
            {property.description && (
              <div style={{ background: "#16161A", border: "1px solid rgba(201,168,76,0.12)", borderRadius: 16, padding: "24px 24px", marginBottom: 24 }}>
                <h3 className="font-kufi" style={{ fontSize: 16, fontWeight: 700, marginBottom: 14, color: "#C9A84C" }}>تفاصيل العقار</h3>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {property.location_url && (
                <a href={property.location_url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 20px", borderRadius: 12, background: "#16161A", border: "1px solid rgba(201,168,76,0.12)", color: "#9A9AA0", textDecoration: "none", fontSize: 13, fontWeight: 500, transition: "all 0.25s" }}>
                  <MapPin size={15} style={{ color: "#C9A84C" }} />
                  الموقع على الخريطة
                </a>
              )}
              {property.images_url && (
                <a href={property.images_url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 20px", borderRadius: 12, background: "#16161A", border: "1px solid rgba(201,168,76,0.12)", color: "#9A9AA0", textDecoration: "none", fontSize: 13, fontWeight: 500, transition: "all 0.25s" }}>
                  <Images size={15} style={{ color: "#C9A84C" }} />
                  مجلد الصور الكامل
                </a>
              )}
            </div>
          ...
          <div style={{ position: "sticky", top: 92 }}>
            <div style={{ background: "#16161A", border: "1px solid rgba(201,168,76,0.18)", borderRadius: 20, overflow: "hidden" }}>
              {/* Price header */}
              <div style={{ background: "linear-gradient(135deg, rgba(201,168,76,0.08), rgba(166,138,58,0.04))", padding: "24px 24px", borderBottom: "1px solid rgba(201,168,76,0.1)" }}>
                {price ? (
                  <>
                    <div style={{ fontSize: 11, color: "#5A5A62", marginBottom: 6, letterSpacing: 0.5 }}>السعر</div>
                    <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                      <span className="font-kufi" style={{ fontSize: 28, fontWeight: 900, color: "#C9A84C" }}>{price}</span>
              ...
              <div style={{ padding: "20px 20px" }}>
                {hasContact && (
                  <>
                    ...
                    <button onClick={handleCall} className="action-btn" style={{ width: "100%", padding: "14px", borderRadius: 14, marginBottom: 10, background: "#1C1C22", border: "1px solid rgba(201,168,76,0.15)", color: "#F5F5F5", fontSize: 14, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontFamily: "'Tajawal', sans-serif" }}>
                      <Phone size={18} style={{ color: "#C9A84C" }} />
                      اتصال مباشر
                    </button>
                  </>
                )}
                <button onClick={handleShare} className="action-btn" style={{ width: "100%", padding: "12px", borderRadius: 12, background: copied ? "rgba(74,222,128,0.08)" : "transparent", border: "1px solid " + (copied ? "rgba(74,222,128,0.2)" : "rgba(201,168,76,0.08)"), color: copied ? "#4ADE80" : "#5A5A62", fontSize: 13, fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "'Tajawal', sans-serif", transition: "all 0.3s" }}>
                  <Share2 size={15} />
              ...
              {/* Agent info */}
              <div style={{ borderTop: "1px solid rgba(201,168,76,0.08)", padding: "16px 20px", display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg, #C9A84C, #A68A3A)", display: "flex", alignItems: "center", justifyContent: "center", color: "#0A0A0C", fontSize: 16, fontWeight: 900, flexShrink: 0, fontFamily: "'Noto Kufi Arabic', serif" }}>إ</div>
                <div>
