...
export async function POST(request: NextRequest) {
  try {
    ...
    // ── Fetch Config ──
    const { data: dbKeysRaw } = await supabase
      .from("ai_config")
    ...
    if (images && images.length > 0 && keys.anthropic) {
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 2000,
          system: EXTRACT_PROMPT,
          messages: [{ role: "user", content: [...imageContent, ...textPart] }],
        }),
      });
      const data = await response.json();
    ...
    if (!extracted)
      return NextResponse.json(
        { error: "تعذّر تحليل المحتوى — حاول بصيغة مختلفة" },
        { status: 422 }
      );
    ...
    if (msg.includes("rate limit") || msg.includes("429"))
      return NextResponse.json({ error: "انتظر قليلاً ثم حاول مجدداً" }, { status: 429 });
