  ...
  try {
    ...
    if (!rateCheck.allowed) {
      return NextResponse.json(
        { error: `عدد الطلبات كبير — انتظر ${rateCheck.retryAfterSeconds} ثانية` },
        { status: 429 }
      );
    }
    ...
    // ── Build AI request ──
    // Try Anthropic first (supports vision), fallback to Google, then OpenAI
    let extracted: any = null;

    if (images && images.length > 0 && process.env.ANTHROPIC_API_KEY) {
      // Vision: send images to Anthropic Claude
      ...
      const textPart = content
        ? [{ type: "text", text: `هذه بيانات إضافية عن العقار:\n${content}` }]
        : [{ type: "text", text: "حلل هذه الصورة واستخرج بيانات العقار منها" }];
      ...
      const result = await callTextAI(content);
      extracted = parseJSON(result);
    }

    if (!extracted) {
      return NextResponse.json({ error: "تعذّر تحليل المحتوى — حاول بصيغة مختلفة" }, { status: 422 });
    }
...
// ── Call text AI (try providers in order) ──
async function callTextAI(content: string): Promise<string> {
  ...
  throw new Error("لا يوجد مزود AI متاح");
}
