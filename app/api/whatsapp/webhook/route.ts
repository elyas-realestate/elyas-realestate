...
async function maybeAutoReply(
  ...
  const systemPrompt = `أنت مساعد رد تلقائي لوسيط عقاري سعودي${identity?.broker_name ? ` اسمه ${identity.broker_name}` : ""}.
...
ردّك:
- قصير (60-100 كلمة كحد أقصى)
- بالعربية الفصحى
- نبرة دافئة احترافية
- بدون emojis إلا واحد كحد أقصى
- لا تختلق معلومات
- إذا فيه عقارات مطابقة، اذكرها بإيجاز واطلب رقم تواصل مفضّل أو طريقة للمتابعة
- إذا لم يكن السؤال عن عقار، اعرض المساعدة العامة (بحث عقار، تقييم، عرض حصري)`;
...
${propsContext ? propsContext + "\n\n" : ""}اكتب الرد المناسب فقط (بدون شرح أو عناوين).`;
  ...
  try {
    reply = await generateText({
      provider: ((cfg.ai_provider as string) || "openai") as AIProvider,
