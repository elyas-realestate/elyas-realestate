...
const log = createLogger({ route: "/api/whatsapp/webhook" });
...
async function processWebhook(body: MetaWebhookPayload) {
  ...
  for (const entry of body.entry || []) {
    for (const change of entry.changes || []) {
      ...
      // اعثر على tenant_id
      const { data: tid } = await admin.rpc("tenant_by_whatsapp_phone_id", {
        pn_id: phoneNumberId,
      });
      if (!tid) {
      ...
      for (const msg of value.messages || []) {
        ...
        sendPushToTenant(tenantId, {
          ...
          tag: `wa-${msg.from}`,
        }).catch((e) => log.warn("Push failed", { error: String(e) }));
...
async function isCEOPhone(
  ...
  if (identity?.phones && Array.isArray(identity.phones)) {
    const match = identity.phones.some((p: { number?: string }) => p?.number === phone);
...
ابدأ ردّك مباشرة بدون مقدمات. لا تقل "أحتاج توجيه".`;
  ...
  try {
    ...
    log.warn("CEO assistant generation failed", { error: String(e) });
...
async function maybeAutoReply(
  ...
  if (!clientGate.ok) {
    await admin.from("org_activity_log").insert({
      tenant_id: tenantId,
      actor_kind: "system",
      action: "whatsapp_client_skipped",
...
${propsContext ? propsContext + "\n\n" : ""}اكتب الرد المناسب فقط (بدون شرح أو عناوين).`;
  ...
  try {
    ...
    log.warn("AI generation failed", { error: String(e) });
