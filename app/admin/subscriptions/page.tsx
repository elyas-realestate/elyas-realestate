  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 24, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>الاشتراكات والإيرادات</h1>
          <p style={{ fontSize: 13, color: "var(--text-disabled)" }}>إيرادات شهرية وسنوية، خطط نشطة، ومدفوعة</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14, marginBottom: 24 }}>
        {[
          { label: "MRR — دخل شهري",  value: summary ? `${summary.mrr.toLocaleString("en-US")} ر.س` : "—", icon: DollarSign, color: "var(--success-2)", bg: "rgba(52,211,153,0.08)" },
          { label: "ARR — دخل سنوي",  value: summary ? `${summary.arr.toLocaleString("en-US")} ر.س` : "—", icon: TrendingUp, color: "var(--gold-1)", bg: "rgba(232,184,109,0.08)" },
          { label: "اشتراكات مدفوعة", value: summary ? summary.paying : "—", icon: Crown,     color: "var(--purple-ai)", bg: "rgba(124,58,237,0.08)" },
          { label: "إجمالي الخطط",    value: summary ? summary.total  : "—", icon: Zap,       color: "var(--info)", bg: "rgba(96,165,250,0.08)" },
      ...
      <div style={{ background: "var(--bg-deep)", border: "1px solid var(--overlay-soft)", borderRadius: 14, overflow: "hidden" }}>
        <div style={{ padding: "14px 18px", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
          <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-muted)" }}>كل الاشتراكات</h2>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 700 }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} style={{ padding: 32, textAlign: "center", color: "var(--text-disabled)" }}>لا توجد اشتراكات</td></tr>
              ) : (
                subs.map(s => {
                  ...
                  return (
                    <tr key={s.tenant_id} style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                      <td style={{ padding: "11px 14px" }}>
                        <Link href={`/admin/tenants/${s.tenant_id}`} style={{ color: "var(--text-on-dark)", fontWeight: 600 }}>
                          {s.broker_name || s.slug}
                        </Link>
                        <div style={{ fontSize: 11, color: "var(--text-disabled)", direction: "ltr", textAlign: "right" }}>/{s.slug}</div>
                      </td>
                      <td style={{ padding: "11px 14px" }}>
                      ...
                      <td style={{ padding: "11px 14px" }}>
                        <span style={{ fontSize: 11, color: s.is_active ? "var(--success)" : "var(--danger)" }}>
