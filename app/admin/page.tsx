  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 28, display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>نظرة عامة</h1>
          <p style={{ fontSize: 13, color: "var(--text-disabled)" }}>إحصائيات المنصة ومراقبة الوسطاء</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14, marginBottom: 24 }}>
        {[
          { label: "إجمالي الوسطاء",   value: data?.total ?? 0,                     icon: Users,       color: "var(--purple-ai)", bg: "rgba(124,58,237,0.08)" },
          { label: "حسابات نشطة",      value: loading ? 0 : totalActive,            icon: TrendingUp,  color: "var(--success)", bg: "rgba(74,222,128,0.08)"  },
          { label: "خطة مجانية",       value: data?.planCounts.free  ?? 0,           icon: Gift,        color: "var(--text-ghost)", bg: "rgba(113,113,122,0.08)" },
          { label: "خطة أساسي + احترافي", value: (data?.planCounts.basic ?? 0) + (data?.planCounts.pro ?? 0), icon: Crown, color: "var(--gold-1)", bg: "rgba(232,184,109,0.08)" },
        ].map(({ label, value, icon: Icon, color, bg }) => (
          <div key={label} style={{ background: "var(--bg-deep)", border: "1px solid var(--overlay-soft)", borderRadius: 14, padding: "18px 16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        {/* توزيع الخطط */}
        <div style={{ background: "var(--bg-deep)", border: "1px solid var(--overlay-soft)", borderRadius: 14, padding: 20 }}>
          <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-muted)", marginBottom: 16 }}>توزيع الخطط</h2>
              ...
              return (
                <div key={id}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                    ...
                    <span style={{ fontSize: 12, color: "var(--text-disabled)" }}>{loading ? "—" : `${count} (${pct}%)`}</span>
                  </div>
        ...
        <div style={{ background: "var(--bg-deep)", border: "1px solid var(--overlay-soft)", borderRadius: 14, padding: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-muted)" }}>آخر المسجّلين</h2>
            <a href="/admin/users" style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--purple-2)", textDecoration: "none" }}>
          ...
          {loading ? (
            ...
            <p style={{ fontSize: 13, color: "var(--text-disabled)", textAlign: "center", padding: "16px 0" }}>لا يوجد مستخدمون بعد</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {recentTenants.map(t => {
                const meta = PLAN_META[t.plan] || PLAN_META.free;
                return (
                  <div key={t.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", borderRadius: 8, background: "var(--bg-surface-2)" }}>
                    <div>
                      <div style={{ fontSize: 13, color: "var(--text-secondary)", fontWeight: 500 }}>{t.broker_name || t.slug}</div>
                      <div style={{ fontSize: 10, color: "var(--text-disabled)", direction: "ltr" }}>/{t.slug}</div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      <span style={{ fontSize: 10, color: meta.color, background: meta.bg, padding: "2px 7px", borderRadius: 5, fontWeight: 600 }}>{meta.label}</span>
                      <span style={{ width: 7, height: 7, borderRadius: "50%", background: t.is_active ? "var(--success)" : "var(--danger)", flexShrink: 0 }} />
                    </div>
                  </div>
                );
              })}
