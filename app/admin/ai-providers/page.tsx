  ...
  return (
    <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 12 }}>
                ...
                {typeof result.balance_usd === "number" && (
                  <Row
                    ...
                    valueColor={
                      result.balance_usd < 1
                        ? "var(--danger)"
                        : result.balance_usd < 5
                          ? "var(--gold-1)"
                          : "var(--success)"
                    }
                  />
                ...
                <Row
                  ...
                  value={PRICING_HINTS[result.provider] || "—"}
                  small
                />
                {result.error && (
                  <div
                ...
                {result.status === "no_key" && (
                  ...
                  >
                    أضف{" "}
                    <code
                      style={{ color: "var(--gold-1)", direction: "ltr", display: "inline-block" }}
                    >
                      {envVarName(result.provider)}
                    </code>{" "}
      ...
      >
        ...
        <br />• <strong>Anthropic + Google + Groq + xAI</strong>: لا يوفّرون API للرصيد — استخدم زر
        "افتح Dashboard" لمراجعة الرصيد عند المزوّد مباشرة
      </div>
      ...
      {summary && (summary.failing > 0 || summary.no_key > 0) && !loading && (
        ...
        >
          ...
          >
            {results
              .filter((r) => r.status === "invalid_key")
              .map((r) => (
                <li key={r.provider}>
                  • <strong style={{ color: "var(--danger)" }}>{r.label}</strong>: المفتاح غير صالح
                  — جدّده في dashboard المزوّد ثم حدّث Vercel
                </li>
              ))}
            {results
              ...
              .map((r) => (
                <li key={r.provider}>
                  • <strong style={{ color: "var(--gold-1)" }}>{r.label}</strong>: تجاوز الحد — زِد
                  الرصيد أو ارفع خطة الاشتراك
                </li>
              ))}
            {results
              .filter((r) => r.status === "no_key")
              .map((r) => (
                <li key={r.provider}>
                  • <strong style={{ color: "var(--text-ghost)" }}>{r.label}</strong>: لا يوجد مفتاح
                  في البيئة (اختياري)
                </li>
              ))}
            {results
              .filter((r) => r.balance_usd !== undefined && r.balance_usd < 5)
              .map((r) => (
                <li key={r.provider}>
                  • <strong style={{ color: "var(--gold-1)" }}>{r.label}</strong>: رصيدك منخفض ($
                  {r.balance_usd?.toFixed(2)}) — اشحن قبل النفاد
                </li>
              ))}
          </ul>
        </div>
