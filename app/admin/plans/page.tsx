...
const PLANS = [
  {
    id: "free", name: "مجاني", price: 0, color: "var(--text-ghost)", icon: Gift,
  ...
  {
    id: "basic", name: "أساسي", price: 99, color: "var(--gold-2)", icon: Zap, badge: "الأكثر طلباً",
    features: ["٥٠ عقاراً", "عملاء غير محدودين", "صفحة هبوط", "وكيل المحتوى (٥٠/شهر)", "الوثائق القانونية", "التحليل المالي"],
    locked: ["غرفة المحتوى"],
  },
  {
    id: "pro", name: "احترافي", price: 249, color: "var(--gold-1)", icon: Crown,
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>الاشتراكات</h1>
        <p style={{ fontSize: 13, color: "var(--text-disabled)" }}>تفاصيل خطط المنصة وميزاتها</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
        {PLANS.map(plan => {
          ...
          return (
            <div
              key={plan.id}
              style={{ background: "var(--bg-deep)", border: `1px solid ${plan.color}22`, borderRadius: 14, padding: 20, position: "relative" }}
            >
              {"badge" in plan && plan.badge && (
                <div style={{ position: "absolute", top: -10, right: 16, fontSize: 10, fontWeight: 700, color: "var(--bg-page)", background: "linear-gradient(135deg, var(--gold-2), var(--gold-3))", padding: "3px 10px", borderRadius: 100 }}>
                  {plan.badge}
                </div>
              )}
              ...
              <div style={{ borderTop: `1px solid ${plan.color}14`, paddingTop: 14 }}>
                <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 7 }}>
                  {plan.features.map((f, i) => (
                    <li key={i} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12, color: "var(--text-secondary)" }}>
                      <Check size={12} style={{ color: "var(--success)", flexShrink: 0 }} />
                      {f}
                    </li>
                  ))}
                  {plan.locked.map((f, i) => (
                    <li key={i} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12, color: "var(--border-1)", textDecoration: "line-through" }}>
                      <X size={12} style={{ color: "var(--border-1)", flexShrink: 0 }} />
                      {f}
                    </li>
                  ))}
                </ul>
      ...
      <div style={{ marginTop: 24, padding: "16px 20px", borderRadius: 12, background: "rgba(124,58,237,0.04)", border: "1px solid rgba(124,58,237,0.12)" }}>
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          <CreditCard size={15} style={{ color: "var(--purple-2)", flexShrink: 0, marginTop: 1 }} />
          <p style={{ fontSize: 12, color: "var(--purple-2)", lineHeight: 1.7 }}>
