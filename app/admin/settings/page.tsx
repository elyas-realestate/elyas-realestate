...
const STATUS_COLORS: Record<string, { bg: string; border: string; text: string; label: string }> = {
  set:     { bg: "rgba(74,222,128,0.05)",  border: "rgba(74,222,128,0.2)",  text: "var(--success)", label: "مضبوط"  },
  missing: { bg: "rgba(239,68,68,0.05)",   border: "rgba(239,68,68,0.2)",   text: "var(--danger)", label: "مفقود"  },
  unknown: { bg: "rgba(161,161,170,0.05)", border: "rgba(161,161,170,0.1)", text: "var(--text-ghost)", label: "غير محدد" },
};
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>إعدادات الإدارة</h1>
        <p style={{ fontSize: 13, color: "var(--text-disabled)" }}>متغيرات البيئة والإعدادات التقنية</p>
      </div>

      {/* ENV Vars */}
      <div style={{ background: "var(--bg-deep)", border: "1px solid var(--overlay-soft)", borderRadius: 14, padding: 20, marginBottom: 16 }}>
        <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-muted)", marginBottom: 16 }}>متغيرات البيئة</h2>
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 12, color: "var(--text-secondary)", fontFamily: "monospace" }}>{v.name}</div>
                    <div style={{ fontSize: 11, color: "var(--text-disabled)" }}>{v.label}</div>
                  </div>
                </div>
        ...
        <p style={{ fontSize: 12, color: "var(--border-1)", marginTop: 14, lineHeight: 1.6 }}>
      ...
      <div style={{ background: "var(--bg-deep)", border: "1px solid var(--overlay-soft)", borderRadius: 14, padding: 20 }}>
        <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-muted)", marginBottom: 12 }}>حماية لوحة الإدارة</h2>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <InfoRow
            label="طريقة الحماية"
...
function InfoRow({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
      <span style={{ fontSize: 12, color: "var(--text-disabled)" }}>{label}</span>
