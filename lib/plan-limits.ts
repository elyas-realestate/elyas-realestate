...
export async function getCurrentPlan(supabase: SupabaseClient): Promise<PlanId> {
  const { data, error } = await supabase.from("site_settings").select("plan").limit(1).single();
...
export async function checkLimit(
  ...
  if (resource === "documents") {
    if (!limits.documents) {
      return {
        allowed: false,
        reason: `الوثائق القانونية غير متاحة في الخطة ${PLAN_NAMES[plan]} — يرجى الترقية`,
      };
    }
    return { allowed: true };
  }
  ...
  if (limit === 0) {
    return {
      allowed: false,
      reason:
        BLOCK_MESSAGES[resource]?.[plan] || `هذه الميزة غير متاحة في الخطة ${PLAN_NAMES[plan]}`,
      current: 0,
      limit: 0,
    };
  }
